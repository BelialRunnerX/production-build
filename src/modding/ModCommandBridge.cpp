// Intended function: Translate validated mod commands into owner-system request envelopes.
#include "modding/ModCommandBridge.hpp"
...
bool ModCommandBridgeService::apply(const ModCommandBridgeCommand& command) {
...
bool ModCommandBridgeService::erase(std::uint64_t subjectId) {
...
void ModCommandBridgeService::advance(std::uint64_t tick, double delta) {
...
const ModCommandBridgeState* ModCommandBridgeService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ModCommandBridgeState* ModCommandBridgeService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ModCommandBridgeState> ModCommandBridgeService::ordered() const {
    auto result = states_;
...
std::vector<ModCommandBridgeEvent> ModCommandBridgeService::drainEvents() {
...
void ModCommandBridgeService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ModCommandBridgeService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
