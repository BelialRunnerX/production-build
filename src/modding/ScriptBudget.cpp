...
#include "modding/ScriptBudget.hpp"
...
bool ScriptBudgetService::apply(const ScriptBudgetCommand& command) {
...
bool ScriptBudgetService::erase(std::uint64_t subjectId) {
...
void ScriptBudgetService::advance(std::uint64_t tick, double delta) {
...
const ScriptBudgetState* ScriptBudgetService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ScriptBudgetState* ScriptBudgetService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ScriptBudgetState> ScriptBudgetService::ordered() const {
    auto result = states_;
...
std::vector<ScriptBudgetEvent> ScriptBudgetService::drainEvents() {
...
void ScriptBudgetService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ScriptBudgetService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
