// Intended function: Describe safe content/script hot-reload operations without mutating live authority directly.
#include "modding/HotReloadPlan.hpp"
...
bool HotReloadPlanService::apply(const HotReloadPlanCommand& command) {
...
bool HotReloadPlanService::erase(std::uint64_t subjectId) {
...
void HotReloadPlanService::advance(std::uint64_t tick, double delta) {
...
const HotReloadPlanState* HotReloadPlanService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
HotReloadPlanState* HotReloadPlanService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<HotReloadPlanState> HotReloadPlanService::ordered() const {
    auto result = states_;
...
std::vector<HotReloadPlanEvent> HotReloadPlanService::drainEvents() {
...
void HotReloadPlanService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void HotReloadPlanService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
