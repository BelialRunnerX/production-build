...
#include "modding/ModPackageIndex.hpp"
...
bool ModPackageIndexService::apply(const ModPackageIndexCommand& command) {
...
bool ModPackageIndexService::erase(std::uint64_t subjectId) {
...
void ModPackageIndexService::advance(std::uint64_t tick, double delta) {
...
const ModPackageIndexState* ModPackageIndexService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ModPackageIndexState* ModPackageIndexService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ModPackageIndexState> ModPackageIndexService::ordered() const {
    auto result = states_;
...
std::vector<ModPackageIndexEvent> ModPackageIndexService::drainEvents() {
...
void ModPackageIndexService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ModPackageIndexService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
