// Intended function: Register versioned mod save/replication schema extensions behind explicit ownership.
#include "modding/SchemaExtensionRegistry.hpp"
...
bool SchemaExtensionRegistryService::apply(const SchemaExtensionRegistryCommand& command) {
...
bool SchemaExtensionRegistryService::erase(std::uint64_t subjectId) {
...
void SchemaExtensionRegistryService::advance(std::uint64_t tick, double delta) {
...
const SchemaExtensionRegistryState* SchemaExtensionRegistryService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SchemaExtensionRegistryState* SchemaExtensionRegistryService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SchemaExtensionRegistryState> SchemaExtensionRegistryService::ordered() const {
    auto result = states_;
...
std::vector<SchemaExtensionRegistryEvent> SchemaExtensionRegistryService::drainEvents() {
...
void SchemaExtensionRegistryService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SchemaExtensionRegistryService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
