// Intended function: Assign stable content namespaces so mod IDs cannot collide with core content.
#include "modding/ContentNamespaceRegistry.hpp"
...
bool ContentNamespaceRegistryService::apply(const ContentNamespaceRegistryCommand& command) {
...
bool ContentNamespaceRegistryService::erase(std::uint64_t subjectId) {
...
void ContentNamespaceRegistryService::advance(std::uint64_t tick, double delta) {
...
const ContentNamespaceRegistryState* ContentNamespaceRegistryService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ContentNamespaceRegistryState* ContentNamespaceRegistryService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ContentNamespaceRegistryState> ContentNamespaceRegistryService::ordered() const {
    auto result = states_;
...
std::vector<ContentNamespaceRegistryEvent> ContentNamespaceRegistryService::drainEvents() {
...
void ContentNamespaceRegistryService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ContentNamespaceRegistryService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
