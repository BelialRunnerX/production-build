// Intended function: Declare which world, UI, scripting, content, and network capabilities each mod may use.
#include "modding/ModCapabilityPolicy.hpp"
...
bool ModCapabilityPolicyService::apply(const ModCapabilityPolicyCommand& command) {
...
bool ModCapabilityPolicyService::erase(std::uint64_t subjectId) {
...
void ModCapabilityPolicyService::advance(std::uint64_t tick, double delta) {
...
const ModCapabilityPolicyState* ModCapabilityPolicyService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ModCapabilityPolicyState* ModCapabilityPolicyService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ModCapabilityPolicyState> ModCapabilityPolicyService::ordered() const {
    auto result = states_;
...
std::vector<ModCapabilityPolicyEvent> ModCapabilityPolicyService::drainEvents() {
...
void ModCapabilityPolicyService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ModCapabilityPolicyService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
