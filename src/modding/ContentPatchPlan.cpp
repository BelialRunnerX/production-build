...
#include "modding/ContentPatchPlan.hpp"
...
bool ContentPatchPlanService::apply(const ContentPatchPlanCommand& command) {
...
bool ContentPatchPlanService::erase(std::uint64_t subjectId) {
...
void ContentPatchPlanService::advance(std::uint64_t tick, double delta) {
...
const ContentPatchPlanState* ContentPatchPlanService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ContentPatchPlanState* ContentPatchPlanService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ContentPatchPlanState> ContentPatchPlanService::ordered() const {
    auto result = states_;
...
std::vector<ContentPatchPlanEvent> ContentPatchPlanService::drainEvents() {
...
void ContentPatchPlanService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ContentPatchPlanService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
