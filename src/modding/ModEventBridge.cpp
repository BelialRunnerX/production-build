// Intended function: Expose curated simulation events to mods without raw registry or pointer access.
#include "modding/ModEventBridge.hpp"
...
bool ModEventBridgeService::apply(const ModEventBridgeCommand& command) {
...
bool ModEventBridgeService::erase(std::uint64_t subjectId) {
...
void ModEventBridgeService::advance(std::uint64_t tick, double delta) {
...
const ModEventBridgeState* ModEventBridgeService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ModEventBridgeState* ModEventBridgeService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ModEventBridgeState> ModEventBridgeService::ordered() const {
    auto result = states_;
...
std::vector<ModEventBridgeEvent> ModEventBridgeService::drainEvents() {
...
void ModEventBridgeService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ModEventBridgeService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
