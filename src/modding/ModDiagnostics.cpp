// Intended function: Collect load, schema, capability, budget, and runtime diagnostics per mod namespace.
#include "modding/ModDiagnostics.hpp"
...
bool ModDiagnosticsService::apply(const ModDiagnosticsCommand& command) {
...
bool ModDiagnosticsService::erase(std::uint64_t subjectId) {
...
void ModDiagnosticsService::advance(std::uint64_t tick, double delta) {
...
const ModDiagnosticsState* ModDiagnosticsService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ModDiagnosticsState* ModDiagnosticsService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ModDiagnosticsState> ModDiagnosticsService::ordered() const {
    auto result = states_;
...
std::vector<ModDiagnosticsEvent> ModDiagnosticsService::drainEvents() {
...
void ModDiagnosticsService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ModDiagnosticsService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
