...
#include "modding/ModMigrationRegistry.hpp"
...
bool ModMigrationRegistryService::apply(const ModMigrationRegistryCommand& command) {
...
bool ModMigrationRegistryService::erase(std::uint64_t subjectId) {
...
void ModMigrationRegistryService::advance(std::uint64_t tick, double delta) {
...
const ModMigrationRegistryState* ModMigrationRegistryService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ModMigrationRegistryState* ModMigrationRegistryService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ModMigrationRegistryState> ModMigrationRegistryService::ordered() const {
    auto result = states_;
...
std::vector<ModMigrationRegistryEvent> ModMigrationRegistryService::drainEvents() {
...
void ModMigrationRegistryService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ModMigrationRegistryService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
