#include "galaxy/JumpNetwork.hpp"
...
bool JumpNetworkSystem::apply(const JumpNetworkRequest& request) {
...
bool JumpNetworkSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const JumpNetworkState* JumpNetworkSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> JumpNetworkSystem::activeIds() const {
