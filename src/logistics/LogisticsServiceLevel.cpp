// Intended function: Track delivery latency, fill rate, loss, congestion, and service-level breaches.
#include "logistics/LogisticsServiceLevel.hpp"
...
bool LogisticsServiceLevelService::apply(const LogisticsServiceLevelCommand& command) {
...
bool LogisticsServiceLevelService::erase(std::uint64_t subjectId) {
...
void LogisticsServiceLevelService::advance(std::uint64_t tick, double delta) {
...
const LogisticsServiceLevelState* LogisticsServiceLevelService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
LogisticsServiceLevelState* LogisticsServiceLevelService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<LogisticsServiceLevelState> LogisticsServiceLevelService::ordered() const {
    auto result = states_;
...
std::vector<LogisticsServiceLevelEvent> LogisticsServiceLevelService::drainEvents() {
...
void LogisticsServiceLevelService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void LogisticsServiceLevelService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
