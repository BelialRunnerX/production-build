...
#include "logistics/ConvoyAssembly.hpp"
...
bool ConvoyAssemblyService::apply(const ConvoyAssemblyCommand& command) {
...
bool ConvoyAssemblyService::erase(std::uint64_t subjectId) {
...
void ConvoyAssemblyService::advance(std::uint64_t tick, double delta) {
...
const ConvoyAssemblyState* ConvoyAssemblyService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ConvoyAssemblyState* ConvoyAssemblyService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ConvoyAssemblyState> ConvoyAssemblyService::ordered() const {
    auto result = states_;
...
std::vector<ConvoyAssemblyEvent> ConvoyAssemblyService::drainEvents() {
...
void ConvoyAssemblyService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ConvoyAssemblyService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
