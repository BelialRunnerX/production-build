...
#include "logistics/HazardousCargo.hpp"
...
bool HazardousCargoService::apply(const HazardousCargoCommand& command) {
...
bool HazardousCargoService::erase(std::uint64_t subjectId) {
...
void HazardousCargoService::advance(std::uint64_t tick, double delta) {
...
const HazardousCargoState* HazardousCargoService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
HazardousCargoState* HazardousCargoService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<HazardousCargoState> HazardousCargoService::ordered() const {
    auto result = states_;
...
std::vector<HazardousCargoEvent> HazardousCargoService::drainEvents() {
...
void HazardousCargoService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void HazardousCargoService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
