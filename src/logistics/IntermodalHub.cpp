...
#include "logistics/IntermodalHub.hpp"
...
bool IntermodalHubService::apply(const IntermodalHubCommand& command) {
...
bool IntermodalHubService::erase(std::uint64_t subjectId) {
...
void IntermodalHubService::advance(std::uint64_t tick, double delta) {
...
const IntermodalHubState* IntermodalHubService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
IntermodalHubState* IntermodalHubService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<IntermodalHubState> IntermodalHubService::ordered() const {
    auto result = states_;
...
std::vector<IntermodalHubEvent> IntermodalHubService::drainEvents() {
...
void IntermodalHubService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void IntermodalHubService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
