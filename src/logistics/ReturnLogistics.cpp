...
#include "logistics/ReturnLogistics.hpp"
...
bool ReturnLogisticsService::apply(const ReturnLogisticsCommand& command) {
...
bool ReturnLogisticsService::erase(std::uint64_t subjectId) {
...
void ReturnLogisticsService::advance(std::uint64_t tick, double delta) {
...
const ReturnLogisticsState* ReturnLogisticsService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ReturnLogisticsState* ReturnLogisticsService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ReturnLogisticsState> ReturnLogisticsService::ordered() const {
    auto result = states_;
...
std::vector<ReturnLogisticsEvent> ReturnLogisticsService::drainEvents() {
...
void ReturnLogisticsService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ReturnLogisticsService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
