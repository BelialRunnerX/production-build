...
#include "logistics/PriorityFreight.hpp"
...
bool PriorityFreightService::apply(const PriorityFreightCommand& command) {
...
bool PriorityFreightService::erase(std::uint64_t subjectId) {
...
void PriorityFreightService::advance(std::uint64_t tick, double delta) {
...
const PriorityFreightState* PriorityFreightService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PriorityFreightState* PriorityFreightService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PriorityFreightState> PriorityFreightService::ordered() const {
    auto result = states_;
...
std::vector<PriorityFreightEvent> PriorityFreightService::drainEvents() {
...
void PriorityFreightService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PriorityFreightService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
