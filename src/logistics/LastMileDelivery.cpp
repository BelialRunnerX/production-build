// Intended function: Plan bounded local delivery from hubs to final stockpiles, machines, or consumers.
#include "logistics/LastMileDelivery.hpp"
...
bool LastMileDeliveryService::apply(const LastMileDeliveryCommand& command) {
...
bool LastMileDeliveryService::erase(std::uint64_t subjectId) {
...
void LastMileDeliveryService::advance(std::uint64_t tick, double delta) {
...
const LastMileDeliveryState* LastMileDeliveryService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
LastMileDeliveryState* LastMileDeliveryService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<LastMileDeliveryState> LastMileDeliveryService::ordered() const {
    auto result = states_;
...
std::vector<LastMileDeliveryEvent> LastMileDeliveryService::drainEvents() {
...
void LastMileDeliveryService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void LastMileDeliveryService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
