...
#include "logistics/RouteCapacity.hpp"
...
bool RouteCapacityService::apply(const RouteCapacityCommand& command) {
...
bool RouteCapacityService::erase(std::uint64_t subjectId) {
...
void RouteCapacityService::advance(std::uint64_t tick, double delta) {
...
const RouteCapacityState* RouteCapacityService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
RouteCapacityState* RouteCapacityService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<RouteCapacityState> RouteCapacityService::ordered() const {
    auto result = states_;
...
std::vector<RouteCapacityEvent> RouteCapacityService::drainEvents() {
...
void RouteCapacityService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void RouteCapacityService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
