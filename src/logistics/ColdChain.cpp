// Intended function: Track temperature-sensitive cargo exposure, refrigeration capacity, and spoilage risk.
#include "logistics/ColdChain.hpp"
...
bool ColdChainService::apply(const ColdChainCommand& command) {
...
bool ColdChainService::erase(std::uint64_t subjectId) {
...
void ColdChainService::advance(std::uint64_t tick, double delta) {
...
const ColdChainState* ColdChainService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ColdChainState* ColdChainService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ColdChainState> ColdChainService::ordered() const {
    auto result = states_;
...
std::vector<ColdChainEvent> ColdChainService::drainEvents() {
...
void ColdChainService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ColdChainService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
