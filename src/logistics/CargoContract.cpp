// Intended function: Track freight commitments, pickup/delivery windows, owners, cargo identity, and penalties.
#include "logistics/CargoContract.hpp"
...
bool CargoContractService::apply(const CargoContractCommand& command) {
...
bool CargoContractService::erase(std::uint64_t subjectId) {
...
void CargoContractService::advance(std::uint64_t tick, double delta) {
...
const CargoContractState* CargoContractService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CargoContractState* CargoContractService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CargoContractState> CargoContractService::ordered() const {
    auto result = states_;
...
std::vector<CargoContractEvent> CargoContractService::drainEvents() {
...
void CargoContractService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CargoContractService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
