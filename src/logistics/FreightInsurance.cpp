...
#include "logistics/FreightInsurance.hpp"
...
bool FreightInsuranceService::apply(const FreightInsuranceCommand& command) {
...
bool FreightInsuranceService::erase(std::uint64_t subjectId) {
...
void FreightInsuranceService::advance(std::uint64_t tick, double delta) {
...
const FreightInsuranceState* FreightInsuranceService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
FreightInsuranceState* FreightInsuranceService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<FreightInsuranceState> FreightInsuranceService::ordered() const {
    auto result = states_;
...
std::vector<FreightInsuranceEvent> FreightInsuranceService::drainEvents() {
...
void FreightInsuranceService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void FreightInsuranceService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
