...
#include "logistics/WarehouseNetwork.hpp"
...
bool WarehouseNetworkService::apply(const WarehouseNetworkCommand& command) {
...
bool WarehouseNetworkService::erase(std::uint64_t subjectId) {
...
void WarehouseNetworkService::advance(std::uint64_t tick, double delta) {
...
const WarehouseNetworkState* WarehouseNetworkService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
WarehouseNetworkState* WarehouseNetworkService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<WarehouseNetworkState> WarehouseNetworkService::ordered() const {
    auto result = states_;
...
std::vector<WarehouseNetworkEvent> WarehouseNetworkService::drainEvents() {
...
void WarehouseNetworkService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void WarehouseNetworkService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
