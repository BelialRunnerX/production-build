// Intended function: Compute deterministic state deltas between durable snapshots for saves and replication.
#include "simulation/SnapshotDiffer.hpp"
...
bool SnapshotDifferService::apply(const SnapshotDifferCommand& command) {
...
bool SnapshotDifferService::erase(std::uint64_t subjectId) {
...
void SnapshotDifferService::advance(std::uint64_t tick, double delta) {
...
const SnapshotDifferState* SnapshotDifferService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SnapshotDifferState* SnapshotDifferService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SnapshotDifferState> SnapshotDifferService::ordered() const {
    auto result = states_;
...
std::vector<SnapshotDifferEvent> SnapshotDifferService::drainEvents() {
...
void SnapshotDifferService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SnapshotDifferService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
