...
#include "simulation/StatePublication.hpp"
...
bool StatePublicationService::apply(const StatePublicationCommand& command) {
...
bool StatePublicationService::erase(std::uint64_t subjectId) {
...
void StatePublicationService::advance(std::uint64_t tick, double delta) {
...
const StatePublicationState* StatePublicationService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
StatePublicationState* StatePublicationService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<StatePublicationState> StatePublicationService::ordered() const {
    auto result = states_;
...
std::vector<StatePublicationEvent> StatePublicationService::drainEvents() {
...
void StatePublicationService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void StatePublicationService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
