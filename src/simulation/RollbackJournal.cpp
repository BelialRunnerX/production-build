// Intended function: Record reversible owner-system mutations for future networking/replay rollback windows.
#include "simulation/RollbackJournal.hpp"
...
bool RollbackJournalService::apply(const RollbackJournalCommand& command) {
...
bool RollbackJournalService::erase(std::uint64_t subjectId) {
...
void RollbackJournalService::advance(std::uint64_t tick, double delta) {
...
const RollbackJournalState* RollbackJournalService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
RollbackJournalState* RollbackJournalService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<RollbackJournalState> RollbackJournalService::ordered() const {
    auto result = states_;
...
std::vector<RollbackJournalEvent> RollbackJournalService::drainEvents() {
...
void RollbackJournalService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void RollbackJournalService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
