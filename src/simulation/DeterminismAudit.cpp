...
#include "simulation/DeterminismAudit.hpp"
...
bool DeterminismAuditService::apply(const DeterminismAuditCommand& command) {
...
bool DeterminismAuditService::erase(std::uint64_t subjectId) {
...
void DeterminismAuditService::advance(std::uint64_t tick, double delta) {
...
const DeterminismAuditState* DeterminismAuditService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DeterminismAuditState* DeterminismAuditService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DeterminismAuditState> DeterminismAuditService::ordered() const {
    auto result = states_;
...
std::vector<DeterminismAuditEvent> DeterminismAuditService::drainEvents() {
...
void DeterminismAuditService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DeterminismAuditService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
