// Intended function: Track fixed-tick, strategic-step, pause, acceleration, and catch-up time domains.
...
bool SimulationClockService::apply(const SimulationClockCommand& command) {
...
bool SimulationClockService::erase(std::uint64_t subjectId) {
...
void SimulationClockService::advance(std::uint64_t tick, double delta) {
...
const SimulationClockState* SimulationClockService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SimulationClockState* SimulationClockService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SimulationClockState> SimulationClockService::ordered() const {
    auto result = states_;
...
std::vector<SimulationClockEvent> SimulationClockService::drainEvents() {
...
void SimulationClockService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SimulationClockService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
