// Intended function: Schedule deterministic simulation phases and owner-system execution order.
#include "simulation/PhaseOrchestrator.hpp"
...
bool PhaseOrchestratorService::apply(const PhaseOrchestratorCommand& command) {
...
bool PhaseOrchestratorService::erase(std::uint64_t subjectId) {
...
void PhaseOrchestratorService::advance(std::uint64_t tick, double delta) {
...
const PhaseOrchestratorState* PhaseOrchestratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PhaseOrchestratorState* PhaseOrchestratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PhaseOrchestratorState> PhaseOrchestratorService::ordered() const {
    auto result = states_;
...
std::vector<PhaseOrchestratorEvent> PhaseOrchestratorService::drainEvents() {
...
void PhaseOrchestratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PhaseOrchestratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
