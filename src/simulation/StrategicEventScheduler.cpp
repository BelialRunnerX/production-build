#include "simulation/StrategicEventScheduler.hpp"
...
bool StrategicEventSchedulerSystem::apply(const StrategicEventSchedulerRequest& request) {
...
bool StrategicEventSchedulerSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const StrategicEventSchedulerState* StrategicEventSchedulerSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> StrategicEventSchedulerSystem::activeIds() const {
...
void StrategicEventSchedulerSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
