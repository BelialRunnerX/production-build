...
#include "simulation/ShardCoordinator.hpp"
...
bool ShardCoordinatorService::apply(const ShardCoordinatorCommand& command) {
...
bool ShardCoordinatorService::erase(std::uint64_t subjectId) {
...
void ShardCoordinatorService::advance(std::uint64_t tick, double delta) {
...
const ShardCoordinatorState* ShardCoordinatorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ShardCoordinatorState* ShardCoordinatorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ShardCoordinatorState> ShardCoordinatorService::ordered() const {
    auto result = states_;
...
std::vector<ShardCoordinatorEvent> ShardCoordinatorService::drainEvents() {
...
void ShardCoordinatorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ShardCoordinatorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
