// Intended function: Issue deterministic per-domain sequence numbers without using transient entity identities.
#include "simulation/StableSequence.hpp"
...
bool StableSequenceService::apply(const StableSequenceCommand& command) {
...
bool StableSequenceService::erase(std::uint64_t subjectId) {
...
void StableSequenceService::advance(std::uint64_t tick, double delta) {
...
const StableSequenceState* StableSequenceService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
StableSequenceState* StableSequenceService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<StableSequenceState> StableSequenceService::ordered() const {
    auto result = states_;
...
std::vector<StableSequenceEvent> StableSequenceService::drainEvents() {
...
void StableSequenceService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void StableSequenceService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
