// Intended function: Queue commands for future ticks using stable sequence ordering and cancellation.
#include "simulation/CommandScheduler.hpp"
...
bool CommandSchedulerService::apply(const CommandSchedulerCommand& command) {
...
bool CommandSchedulerService::erase(std::uint64_t subjectId) {
...
void CommandSchedulerService::advance(std::uint64_t tick, double delta) {
...
const CommandSchedulerState* CommandSchedulerService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CommandSchedulerState* CommandSchedulerService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CommandSchedulerState> CommandSchedulerService::ordered() const {
    auto result = states_;
...
std::vector<CommandSchedulerEvent> CommandSchedulerService::drainEvents() {
...
void CommandSchedulerService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
