...
#include "simulation/BudgetGovernor.hpp"
...
bool BudgetGovernorService::apply(const BudgetGovernorCommand& command) {
...
bool BudgetGovernorService::erase(std::uint64_t subjectId) {
...
void BudgetGovernorService::advance(std::uint64_t tick, double delta) {
...
const BudgetGovernorState* BudgetGovernorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
BudgetGovernorState* BudgetGovernorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<BudgetGovernorState> BudgetGovernorService::ordered() const {
    auto result = states_;
...
std::vector<BudgetGovernorEvent> BudgetGovernorService::drainEvents() {
...
void BudgetGovernorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void BudgetGovernorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
