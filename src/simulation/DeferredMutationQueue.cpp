// Intended function: Collect cross-system mutation requests and publish them at owner-authorized barriers.
#include "simulation/DeferredMutationQueue.hpp"
...
bool DeferredMutationQueueService::apply(const DeferredMutationQueueCommand& command) {
...
bool DeferredMutationQueueService::erase(std::uint64_t subjectId) {
...
void DeferredMutationQueueService::advance(std::uint64_t tick, double delta) {
...
const DeferredMutationQueueState* DeferredMutationQueueService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DeferredMutationQueueState* DeferredMutationQueueService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DeferredMutationQueueState> DeferredMutationQueueService::ordered() const {
    auto result = states_;
...
std::vector<DeferredMutationQueueEvent> DeferredMutationQueueService::drainEvents() {
...
void DeferredMutationQueueService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DeferredMutationQueueService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
