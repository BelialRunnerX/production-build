...
#include "warfare/WarExhaustion.hpp"
...
bool WarExhaustionService::apply(const WarExhaustionCommand& command) {
...
bool WarExhaustionService::erase(std::uint64_t subjectId) {
...
void WarExhaustionService::advance(std::uint64_t tick, double delta) {
...
const WarExhaustionState* WarExhaustionService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
WarExhaustionState* WarExhaustionService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<WarExhaustionState> WarExhaustionService::ordered() const {
    auto result = states_;
...
std::vector<WarExhaustionEvent> WarExhaustionService::drainEvents() {
...
void WarExhaustionService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void WarExhaustionService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
