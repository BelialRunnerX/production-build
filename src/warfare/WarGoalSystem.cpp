...
#include "warfare/WarGoalSystem.hpp"
...
bool WarGoalSystemService::apply(const WarGoalSystemCommand& command) {
...
bool WarGoalSystemService::erase(std::uint64_t subjectId) {
...
void WarGoalSystemService::advance(std::uint64_t tick, double delta) {
...
const WarGoalSystemState* WarGoalSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
WarGoalSystemState* WarGoalSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<WarGoalSystemState> WarGoalSystemService::ordered() const {
    auto result = states_;
...
std::vector<WarGoalSystemEvent> WarGoalSystemService::drainEvents() {
...
void WarGoalSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void WarGoalSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
