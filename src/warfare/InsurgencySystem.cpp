...
#include "warfare/InsurgencySystem.hpp"
...
bool InsurgencySystemService::apply(const InsurgencySystemCommand& command) {
...
bool InsurgencySystemService::erase(std::uint64_t subjectId) {
...
void InsurgencySystemService::advance(std::uint64_t tick, double delta) {
...
const InsurgencySystemState* InsurgencySystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
InsurgencySystemState* InsurgencySystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<InsurgencySystemState> InsurgencySystemService::ordered() const {
    auto result = states_;
...
std::vector<InsurgencySystemEvent> InsurgencySystemService::drainEvents() {
...
void InsurgencySystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void InsurgencySystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
