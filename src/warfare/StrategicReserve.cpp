// Intended function: Track reserve forces, mobilization readiness, reinforcement latency, and replenishment.
#include "warfare/StrategicReserve.hpp"
...
bool StrategicReserveService::apply(const StrategicReserveCommand& command) {
...
bool StrategicReserveService::erase(std::uint64_t subjectId) {
...
void StrategicReserveService::advance(std::uint64_t tick, double delta) {
...
const StrategicReserveState* StrategicReserveService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
StrategicReserveState* StrategicReserveService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<StrategicReserveState> StrategicReserveService::ordered() const {
    auto result = states_;
...
std::vector<StrategicReserveEvent> StrategicReserveService::drainEvents() {
...
void StrategicReserveService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void StrategicReserveService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
