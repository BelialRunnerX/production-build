// Intended function: Model orbital blockade coverage, leakage, inspection, starvation pressure, and relief attempts.
#include "warfare/PlanetaryBlockade.hpp"
...
bool PlanetaryBlockadeService::apply(const PlanetaryBlockadeCommand& command) {
...
bool PlanetaryBlockadeService::erase(std::uint64_t subjectId) {
...
void PlanetaryBlockadeService::advance(std::uint64_t tick, double delta) {
...
const PlanetaryBlockadeState* PlanetaryBlockadeService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PlanetaryBlockadeState* PlanetaryBlockadeService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PlanetaryBlockadeState> PlanetaryBlockadeService::ordered() const {
    auto result = states_;
...
std::vector<PlanetaryBlockadeEvent> PlanetaryBlockadeService::drainEvents() {
...
void PlanetaryBlockadeService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PlanetaryBlockadeService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
