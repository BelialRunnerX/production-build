...
#include "warfare/StrategicSupplyLines.hpp"
...
bool StrategicSupplyLinesService::apply(const StrategicSupplyLinesCommand& command) {
...
bool StrategicSupplyLinesService::erase(std::uint64_t subjectId) {
...
void StrategicSupplyLinesService::advance(std::uint64_t tick, double delta) {
...
const StrategicSupplyLinesState* StrategicSupplyLinesService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
StrategicSupplyLinesState* StrategicSupplyLinesService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<StrategicSupplyLinesState> StrategicSupplyLinesService::ordered() const {
    auto result = states_;
...
std::vector<StrategicSupplyLinesEvent> StrategicSupplyLinesService::drainEvents() {
...
void StrategicSupplyLinesService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void StrategicSupplyLinesService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
