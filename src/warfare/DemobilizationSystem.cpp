// Intended function: Convert wartime forces and production back to civilian use while preserving veterans and materiel.
#include "warfare/DemobilizationSystem.hpp"
...
bool DemobilizationSystemService::apply(const DemobilizationSystemCommand& command) {
...
bool DemobilizationSystemService::erase(std::uint64_t subjectId) {
...
void DemobilizationSystemService::advance(std::uint64_t tick, double delta) {
...
const DemobilizationSystemState* DemobilizationSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DemobilizationSystemState* DemobilizationSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DemobilizationSystemState> DemobilizationSystemService::ordered() const {
    auto result = states_;
...
std::vector<DemobilizationSystemEvent> DemobilizationSystemService::drainEvents() {
...
void DemobilizationSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DemobilizationSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
