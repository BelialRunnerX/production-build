...
#include "warfare/TheaterCommand.hpp"
...
bool TheaterCommandService::apply(const TheaterCommandCommand& command) {
...
bool TheaterCommandService::erase(std::uint64_t subjectId) {
...
void TheaterCommandService::advance(std::uint64_t tick, double delta) {
...
const TheaterCommandState* TheaterCommandService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
TheaterCommandState* TheaterCommandService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<TheaterCommandState> TheaterCommandService::ordered() const {
    auto result = states_;
...
std::vector<TheaterCommandEvent> TheaterCommandService::drainEvents() {
...
void TheaterCommandService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void TheaterCommandService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
