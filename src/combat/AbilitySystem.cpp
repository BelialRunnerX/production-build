...
bool AbilitySystemService::submit(const AbilitySystemCommand& command) {
...
const AbilitySystemRecord* AbilitySystemService::lookup(std::uint64_t subjectId) const {
    auto it = records_.find(subjectId);
    return it == records_.end() ? nullptr : &it->second;
}

std::vector<AbilitySystemRecord> AbilitySystemService::snapshot() const {
    std::vector<AbilitySystemRecord> out; out.reserve(records_.size());
...
bool AbilitySystemService::remove(std::uint64_t subjectId) { return records_.erase(subjectId) != 0; }
void AbilitySystemService::reset() { records_.clear(); revision_ = 1; }
