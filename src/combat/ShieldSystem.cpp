#include "combat/ShieldSystem.hpp"
...
bool ShieldSystemService::submit(const ShieldSystemCommand& command) {
...
const ShieldSystemRecord* ShieldSystemService::lookup(std::uint64_t subjectId) const {
    auto it = records_.find(subjectId);
    return it == records_.end() ? nullptr : &it->second;
}

std::vector<ShieldSystemRecord> ShieldSystemService::snapshot() const {
    std::vector<ShieldSystemRecord> out; out.reserve(records_.size());
...
bool ShieldSystemService::remove(std::uint64_t subjectId) { return records_.erase(subjectId) != 0; }
void ShieldSystemService::reset() { records_.clear(); revision_ = 1; }
