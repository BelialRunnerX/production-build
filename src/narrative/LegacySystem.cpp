...
#include "narrative/LegacySystem.hpp"
...
bool LegacySystemService::apply(const LegacySystemCommand& command) {
...
bool LegacySystemService::erase(std::uint64_t subjectId) {
...
void LegacySystemService::advance(std::uint64_t tick, double delta) {
...
const LegacySystemState* LegacySystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
LegacySystemState* LegacySystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<LegacySystemState> LegacySystemService::ordered() const {
    auto result = states_;
...
std::vector<LegacySystemEvent> LegacySystemService::drainEvents() {
...
void LegacySystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void LegacySystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
