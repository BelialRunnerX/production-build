...
#include "narrative/SecretSystem.hpp"
...
bool SecretSystemService::apply(const SecretSystemCommand& command) {
...
bool SecretSystemService::erase(std::uint64_t subjectId) {
...
void SecretSystemService::advance(std::uint64_t tick, double delta) {
...
const SecretSystemState* SecretSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SecretSystemState* SecretSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SecretSystemState> SecretSystemService::ordered() const {
    auto result = states_;
...
std::vector<SecretSystemEvent> SecretSystemService::drainEvents() {
...
void SecretSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SecretSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
