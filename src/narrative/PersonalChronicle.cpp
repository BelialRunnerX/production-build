...
#include "narrative/PersonalChronicle.hpp"
...
bool PersonalChronicleService::apply(const PersonalChronicleCommand& command) {
...
bool PersonalChronicleService::erase(std::uint64_t subjectId) {
...
void PersonalChronicleService::advance(std::uint64_t tick, double delta) {
...
const PersonalChronicleState* PersonalChronicleService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PersonalChronicleState* PersonalChronicleService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PersonalChronicleState> PersonalChronicleService::ordered() const {
    auto result = states_;
...
std::vector<PersonalChronicleEvent> PersonalChronicleService::drainEvents() {
...
void PersonalChronicleService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PersonalChronicleService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
