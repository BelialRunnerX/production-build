...
#include "narrative/DynamicDialogue.hpp"
...
bool DynamicDialogueService::apply(const DynamicDialogueCommand& command) {
...
bool DynamicDialogueService::erase(std::uint64_t subjectId) {
...
void DynamicDialogueService::advance(std::uint64_t tick, double delta) {
...
const DynamicDialogueState* DynamicDialogueService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DynamicDialogueState* DynamicDialogueService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DynamicDialogueState> DynamicDialogueService::ordered() const {
    auto result = states_;
...
std::vector<DynamicDialogueEvent> DynamicDialogueService::drainEvents() {
...
void DynamicDialogueService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DynamicDialogueService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
