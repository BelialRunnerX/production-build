...
#include "narrative/RumorNetwork.hpp"
...
bool RumorNetworkService::apply(const RumorNetworkCommand& command) {
...
bool RumorNetworkService::erase(std::uint64_t subjectId) {
...
void RumorNetworkService::advance(std::uint64_t tick, double delta) {
...
const RumorNetworkState* RumorNetworkService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
RumorNetworkState* RumorNetworkService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<RumorNetworkState> RumorNetworkService::ordered() const {
    auto result = states_;
...
std::vector<RumorNetworkEvent> RumorNetworkService::drainEvents() {
...
void RumorNetworkService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void RumorNetworkService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
