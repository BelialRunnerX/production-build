...
#include "narrative/ProphecySystem.hpp"
...
bool ProphecySystemService::apply(const ProphecySystemCommand& command) {
...
bool ProphecySystemService::erase(std::uint64_t subjectId) {
...
void ProphecySystemService::advance(std::uint64_t tick, double delta) {
...
const ProphecySystemState* ProphecySystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ProphecySystemState* ProphecySystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ProphecySystemState> ProphecySystemService::ordered() const {
    auto result = states_;
...
std::vector<ProphecySystemEvent> ProphecySystemService::drainEvents() {
...
void ProphecySystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ProphecySystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
