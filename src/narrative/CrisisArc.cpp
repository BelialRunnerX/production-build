// Intended function: Track escalating crises with warning, response, consequence, and recovery stages.
#include "narrative/CrisisArc.hpp"
...
bool CrisisArcService::apply(const CrisisArcCommand& command) {
...
bool CrisisArcService::erase(std::uint64_t subjectId) {
...
void CrisisArcService::advance(std::uint64_t tick, double delta) {
...
const CrisisArcState* CrisisArcService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CrisisArcState* CrisisArcService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CrisisArcState> CrisisArcService::ordered() const {
    auto result = states_;
...
std::vector<CrisisArcEvent> CrisisArcService::drainEvents() {
...
void CrisisArcService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CrisisArcService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
