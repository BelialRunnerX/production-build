// Intended function: Create memorial records from significant deaths, disasters, battles, and settlement history.
#include "narrative/MemorialSystem.hpp"
...
bool MemorialSystemService::apply(const MemorialSystemCommand& command) {
...
bool MemorialSystemService::erase(std::uint64_t subjectId) {
...
void MemorialSystemService::advance(std::uint64_t tick, double delta) {
...
const MemorialSystemState* MemorialSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
MemorialSystemState* MemorialSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<MemorialSystemState> MemorialSystemService::ordered() const {
    auto result = states_;
...
std::vector<MemorialSystemEvent> MemorialSystemService::drainEvents() {
...
void MemorialSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void MemorialSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
