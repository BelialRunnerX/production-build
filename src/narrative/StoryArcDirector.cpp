// Intended function: Track multi-stage systemic story arcs driven by actual world facts and durable actors.
#include "narrative/StoryArcDirector.hpp"
...
bool StoryArcDirectorService::apply(const StoryArcDirectorCommand& command) {
...
bool StoryArcDirectorService::erase(std::uint64_t subjectId) {
...
void StoryArcDirectorService::advance(std::uint64_t tick, double delta) {
...
const StoryArcDirectorState* StoryArcDirectorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
StoryArcDirectorState* StoryArcDirectorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<StoryArcDirectorState> StoryArcDirectorService::ordered() const {
    auto result = states_;
...
std::vector<StoryArcDirectorEvent> StoryArcDirectorService::drainEvents() {
...
void StoryArcDirectorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void StoryArcDirectorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
