...
#include "narrative/HistoricalDebate.hpp"
...
bool HistoricalDebateService::apply(const HistoricalDebateCommand& command) {
...
bool HistoricalDebateService::erase(std::uint64_t subjectId) {
...
void HistoricalDebateService::advance(std::uint64_t tick, double delta) {
...
const HistoricalDebateState* HistoricalDebateService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
HistoricalDebateState* HistoricalDebateService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<HistoricalDebateState> HistoricalDebateService::ordered() const {
    auto result = states_;
...
std::vector<HistoricalDebateEvent> HistoricalDebateService::drainEvents() {
...
void HistoricalDebateService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void HistoricalDebateService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
