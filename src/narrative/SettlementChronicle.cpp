...
#include "narrative/SettlementChronicle.hpp"
...
bool SettlementChronicleService::apply(const SettlementChronicleCommand& command) {
...
bool SettlementChronicleService::erase(std::uint64_t subjectId) {
...
void SettlementChronicleService::advance(std::uint64_t tick, double delta) {
...
const SettlementChronicleState* SettlementChronicleService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SettlementChronicleState* SettlementChronicleService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SettlementChronicleState> SettlementChronicleService::ordered() const {
    auto result = states_;
...
std::vector<SettlementChronicleEvent> SettlementChronicleService::drainEvents() {
...
void SettlementChronicleService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SettlementChronicleService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
