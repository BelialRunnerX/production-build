...
#include "narrative/WitnessSystem.hpp"
...
bool WitnessSystemService::apply(const WitnessSystemCommand& command) {
...
bool WitnessSystemService::erase(std::uint64_t subjectId) {
...
void WitnessSystemService::advance(std::uint64_t tick, double delta) {
...
const WitnessSystemState* WitnessSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
WitnessSystemState* WitnessSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<WitnessSystemState> WitnessSystemService::ordered() const {
    auto result = states_;
...
std::vector<WitnessSystemEvent> WitnessSystemService::drainEvents() {
...
void WitnessSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void WitnessSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
