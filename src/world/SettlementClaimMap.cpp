#include "world/SettlementClaimMap.hpp"
...
bool SettlementClaimMapSystem::apply(const SettlementClaimMapRequest& request) {
...
bool SettlementClaimMapSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const SettlementClaimMapState* SettlementClaimMapSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> SettlementClaimMapSystem::activeIds() const {
...
void SettlementClaimMapSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
