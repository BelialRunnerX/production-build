#include "world/TerraformingSystem.hpp"
...
bool TerraformingSystemSystem::apply(const TerraformingSystemRequest& request) {
...
bool TerraformingSystemSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const TerraformingSystemState* TerraformingSystemSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> TerraformingSystemSystem::activeIds() const {
...
void TerraformingSystemSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
