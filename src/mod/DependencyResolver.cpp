#include "mod/DependencyResolver.hpp"
...
bool DependencyResolverSystem::apply(const DependencyResolverRequest& request) {
...
bool DependencyResolverSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const DependencyResolverState* DependencyResolverSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> DependencyResolverSystem::activeIds() const {
...
void DependencyResolverSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
