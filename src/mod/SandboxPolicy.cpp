...
bool SandboxPolicySystem::apply(const SandboxPolicyRequest& request) {
...
bool SandboxPolicySystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const SandboxPolicyState* SandboxPolicySystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> SandboxPolicySystem::activeIds() const {
...
void SandboxPolicySystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
