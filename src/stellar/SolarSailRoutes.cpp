...
#include "stellar/SolarSailRoutes.hpp"
...
bool SolarSailRoutesService::apply(const SolarSailRoutesCommand& command) {
...
bool SolarSailRoutesService::erase(std::uint64_t subjectId) {
...
void SolarSailRoutesService::advance(std::uint64_t tick, double delta) {
...
const SolarSailRoutesState* SolarSailRoutesService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SolarSailRoutesState* SolarSailRoutesService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SolarSailRoutesState> SolarSailRoutesService::ordered() const {
    auto result = states_;
...
std::vector<SolarSailRoutesEvent> SolarSailRoutesService::drainEvents() {
...
void SolarSailRoutesService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SolarSailRoutesService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
