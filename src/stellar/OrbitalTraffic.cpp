...
#include "stellar/OrbitalTraffic.hpp"
...
bool OrbitalTrafficService::apply(const OrbitalTrafficCommand& command) {
...
bool OrbitalTrafficService::erase(std::uint64_t subjectId) {
...
void OrbitalTrafficService::advance(std::uint64_t tick, double delta) {
...
const OrbitalTrafficState* OrbitalTrafficService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
OrbitalTrafficState* OrbitalTrafficService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<OrbitalTrafficState> OrbitalTrafficService::ordered() const {
    auto result = states_;
...
std::vector<OrbitalTrafficEvent> OrbitalTrafficService::drainEvents() {
...
void OrbitalTrafficService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void OrbitalTrafficService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
