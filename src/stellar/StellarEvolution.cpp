...
#include "stellar/StellarEvolution.hpp"
...
bool StellarEvolutionService::apply(const StellarEvolutionCommand& command) {
...
bool StellarEvolutionService::erase(std::uint64_t subjectId) {
...
void StellarEvolutionService::advance(std::uint64_t tick, double delta) {
...
const StellarEvolutionState* StellarEvolutionService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
StellarEvolutionState* StellarEvolutionService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<StellarEvolutionState> StellarEvolutionService::ordered() const {
    auto result = states_;
...
std::vector<StellarEvolutionEvent> StellarEvolutionService::drainEvents() {
...
void StellarEvolutionService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void StellarEvolutionService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
