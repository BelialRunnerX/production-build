...
#include "stellar/RadiationBelts.hpp"
...
bool RadiationBeltsService::apply(const RadiationBeltsCommand& command) {
...
bool RadiationBeltsService::erase(std::uint64_t subjectId) {
...
void RadiationBeltsService::advance(std::uint64_t tick, double delta) {
...
const RadiationBeltsState* RadiationBeltsService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
RadiationBeltsState* RadiationBeltsService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<RadiationBeltsState> RadiationBeltsService::ordered() const {
    auto result = states_;
...
std::vector<RadiationBeltsEvent> RadiationBeltsService::drainEvents() {
...
void RadiationBeltsService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void RadiationBeltsService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
