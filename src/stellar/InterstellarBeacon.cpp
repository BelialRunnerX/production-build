// Intended function: Track navigation beacons, calibration, faction ownership, signal strength, and sabotage state.
#include "stellar/InterstellarBeacon.hpp"
...
bool InterstellarBeaconService::apply(const InterstellarBeaconCommand& command) {
...
bool InterstellarBeaconService::erase(std::uint64_t subjectId) {
...
void InterstellarBeaconService::advance(std::uint64_t tick, double delta) {
...
const InterstellarBeaconState* InterstellarBeaconService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
InterstellarBeaconState* InterstellarBeaconService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<InterstellarBeaconState> InterstellarBeaconService::ordered() const {
    auto result = states_;
...
std::vector<InterstellarBeaconEvent> InterstellarBeaconService::drainEvents() {
...
void InterstellarBeaconService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void InterstellarBeaconService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
