...
#include "stellar/DeepSpaceRelay.hpp"
...
bool DeepSpaceRelayService::apply(const DeepSpaceRelayCommand& command) {
...
bool DeepSpaceRelayService::erase(std::uint64_t subjectId) {
...
void DeepSpaceRelayService::advance(std::uint64_t tick, double delta) {
...
const DeepSpaceRelayState* DeepSpaceRelayService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DeepSpaceRelayState* DeepSpaceRelayService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DeepSpaceRelayState> DeepSpaceRelayService::ordered() const {
    auto result = states_;
...
std::vector<DeepSpaceRelayEvent> DeepSpaceRelayService::drainEvents() {
...
void DeepSpaceRelayService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DeepSpaceRelayService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
