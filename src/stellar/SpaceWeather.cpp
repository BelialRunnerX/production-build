// Intended function: Generate deterministic stellar storms, radiation events, communications interference, and warnings.
#include "stellar/SpaceWeather.hpp"
...
bool SpaceWeatherService::apply(const SpaceWeatherCommand& command) {
...
bool SpaceWeatherService::erase(std::uint64_t subjectId) {
...
void SpaceWeatherService::advance(std::uint64_t tick, double delta) {
...
const SpaceWeatherState* SpaceWeatherService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SpaceWeatherState* SpaceWeatherService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SpaceWeatherState> SpaceWeatherService::ordered() const {
    auto result = states_;
...
std::vector<SpaceWeatherEvent> SpaceWeatherService::drainEvents() {
...
void SpaceWeatherService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SpaceWeatherService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
