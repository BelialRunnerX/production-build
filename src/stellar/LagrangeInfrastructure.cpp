...
#include "stellar/LagrangeInfrastructure.hpp"
...
bool LagrangeInfrastructureService::apply(const LagrangeInfrastructureCommand& command) {
...
bool LagrangeInfrastructureService::erase(std::uint64_t subjectId) {
...
void LagrangeInfrastructureService::advance(std::uint64_t tick, double delta) {
...
const LagrangeInfrastructureState* LagrangeInfrastructureService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
LagrangeInfrastructureState* LagrangeInfrastructureService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<LagrangeInfrastructureState> LagrangeInfrastructureService::ordered() const {
    auto result = states_;
...
std::vector<LagrangeInfrastructureEvent> LagrangeInfrastructureService::drainEvents() {
...
void LagrangeInfrastructureService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void LagrangeInfrastructureService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
