...
#include "stellar/CometForecast.hpp"
...
bool CometForecastService::apply(const CometForecastCommand& command) {
...
bool CometForecastService::erase(std::uint64_t subjectId) {
...
void CometForecastService::advance(std::uint64_t tick, double delta) {
...
const CometForecastState* CometForecastService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CometForecastState* CometForecastService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CometForecastState> CometForecastService::ordered() const {
    auto result = states_;
...
std::vector<CometForecastEvent> CometForecastService::drainEvents() {
...
void CometForecastService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CometForecastService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
