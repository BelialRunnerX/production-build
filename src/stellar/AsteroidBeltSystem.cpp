...
#include "stellar/AsteroidBeltSystem.hpp"
...
bool AsteroidBeltSystemService::apply(const AsteroidBeltSystemCommand& command) {
...
bool AsteroidBeltSystemService::erase(std::uint64_t subjectId) {
...
void AsteroidBeltSystemService::advance(std::uint64_t tick, double delta) {
...
const AsteroidBeltSystemState* AsteroidBeltSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
AsteroidBeltSystemState* AsteroidBeltSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<AsteroidBeltSystemState> AsteroidBeltSystemService::ordered() const {
    auto result = states_;
...
std::vector<AsteroidBeltSystemEvent> AsteroidBeltSystemService::drainEvents() {
...
void AsteroidBeltSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void AsteroidBeltSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
