...
#include "stellar/GravitationalAssist.hpp"
...
bool GravitationalAssistService::apply(const GravitationalAssistCommand& command) {
...
bool GravitationalAssistService::erase(std::uint64_t subjectId) {
...
void GravitationalAssistService::advance(std::uint64_t tick, double delta) {
...
const GravitationalAssistState* GravitationalAssistService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
GravitationalAssistState* GravitationalAssistService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<GravitationalAssistState> GravitationalAssistService::ordered() const {
    auto result = states_;
...
std::vector<GravitationalAssistEvent> GravitationalAssistService::drainEvents() {
...
void GravitationalAssistService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void GravitationalAssistService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
