...
#include "stellar/OrbitalDebris.hpp"
...
bool OrbitalDebrisService::apply(const OrbitalDebrisCommand& command) {
...
bool OrbitalDebrisService::erase(std::uint64_t subjectId) {
...
void OrbitalDebrisService::advance(std::uint64_t tick, double delta) {
...
const OrbitalDebrisState* OrbitalDebrisService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
OrbitalDebrisState* OrbitalDebrisService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<OrbitalDebrisState> OrbitalDebrisService::ordered() const {
    auto result = states_;
...
std::vector<OrbitalDebrisEvent> OrbitalDebrisService::drainEvents() {
...
void OrbitalDebrisService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void OrbitalDebrisService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
