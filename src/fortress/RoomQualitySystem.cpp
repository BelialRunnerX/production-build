#include "fortress/RoomQualitySystem.hpp"
...
bool RoomQualitySystemSystem::apply(const RoomQualitySystemRequest& request) {
...
bool RoomQualitySystemSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const RoomQualitySystemState* RoomQualitySystemSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> RoomQualitySystemSystem::activeIds() const {
...
void RoomQualitySystemSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
