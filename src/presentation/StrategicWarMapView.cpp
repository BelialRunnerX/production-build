...
#include "presentation/StrategicWarMapView.hpp"
...
bool StrategicWarMapViewService::apply(const StrategicWarMapViewCommand& command) {
...
bool StrategicWarMapViewService::erase(std::uint64_t subjectId) {
...
void StrategicWarMapViewService::advance(std::uint64_t tick, double delta) {
...
const StrategicWarMapViewState* StrategicWarMapViewService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
StrategicWarMapViewState* StrategicWarMapViewService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<StrategicWarMapViewState> StrategicWarMapViewService::ordered() const {
    auto result = states_;
...
std::vector<StrategicWarMapViewEvent> StrategicWarMapViewService::drainEvents() {
...
void StrategicWarMapViewService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void StrategicWarMapViewService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
