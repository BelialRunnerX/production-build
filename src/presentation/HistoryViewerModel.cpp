// Intended function: Project Chronicle timelines, filters, cross-links, people, sites, factions, and artifacts.
#include "presentation/HistoryViewerModel.hpp"
...
bool HistoryViewerModelService::apply(const HistoryViewerModelCommand& command) {
...
bool HistoryViewerModelService::erase(std::uint64_t subjectId) {
...
void HistoryViewerModelService::advance(std::uint64_t tick, double delta) {
...
const HistoryViewerModelState* HistoryViewerModelService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
HistoryViewerModelState* HistoryViewerModelService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<HistoryViewerModelState> HistoryViewerModelService::ordered() const {
    auto result = states_;
...
std::vector<HistoryViewerModelEvent> HistoryViewerModelService::drainEvents() {
...
void HistoryViewerModelService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void HistoryViewerModelService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
