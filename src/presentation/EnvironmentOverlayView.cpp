...
#include "presentation/EnvironmentOverlayView.hpp"
...
bool EnvironmentOverlayViewService::apply(const EnvironmentOverlayViewCommand& command) {
...
bool EnvironmentOverlayViewService::erase(std::uint64_t subjectId) {
...
void EnvironmentOverlayViewService::advance(std::uint64_t tick, double delta) {
...
const EnvironmentOverlayViewState* EnvironmentOverlayViewService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
EnvironmentOverlayViewState* EnvironmentOverlayViewService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<EnvironmentOverlayViewState> EnvironmentOverlayViewService::ordered() const {
    auto result = states_;
...
std::vector<EnvironmentOverlayViewEvent> EnvironmentOverlayViewService::drainEvents() {
...
void EnvironmentOverlayViewService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void EnvironmentOverlayViewService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
