...
#include "presentation/CitizenInspectorView.hpp"
...
bool CitizenInspectorViewService::apply(const CitizenInspectorViewCommand& command) {
...
bool CitizenInspectorViewService::erase(std::uint64_t subjectId) {
...
void CitizenInspectorViewService::advance(std::uint64_t tick, double delta) {
...
const CitizenInspectorViewState* CitizenInspectorViewService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CitizenInspectorViewState* CitizenInspectorViewService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CitizenInspectorViewState> CitizenInspectorViewService::ordered() const {
    auto result = states_;
...
std::vector<CitizenInspectorViewEvent> CitizenInspectorViewService::drainEvents() {
...
void CitizenInspectorViewService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CitizenInspectorViewService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
