...
#include "presentation/PhotoModeModel.hpp"
...
bool PhotoModeModelService::apply(const PhotoModeModelCommand& command) {
...
bool PhotoModeModelService::erase(std::uint64_t subjectId) {
...
void PhotoModeModelService::advance(std::uint64_t tick, double delta) {
...
const PhotoModeModelState* PhotoModeModelService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PhotoModeModelState* PhotoModeModelService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PhotoModeModelState> PhotoModeModelService::ordered() const {
    auto result = states_;
...
std::vector<PhotoModeModelEvent> PhotoModeModelService::drainEvents() {
...
void PhotoModeModelService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PhotoModeModelService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
