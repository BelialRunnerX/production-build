...
#include "presentation/ColonyOperationsView.hpp"
...
bool ColonyOperationsViewService::apply(const ColonyOperationsViewCommand& command) {
...
bool ColonyOperationsViewService::erase(std::uint64_t subjectId) {
...
void ColonyOperationsViewService::advance(std::uint64_t tick, double delta) {
...
const ColonyOperationsViewState* ColonyOperationsViewService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ColonyOperationsViewState* ColonyOperationsViewService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ColonyOperationsViewState> ColonyOperationsViewService::ordered() const {
    auto result = states_;
...
std::vector<ColonyOperationsViewEvent> ColonyOperationsViewService::drainEvents() {
...
void ColonyOperationsViewService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ColonyOperationsViewService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
