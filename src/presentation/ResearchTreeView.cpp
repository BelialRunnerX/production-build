...
#include "presentation/ResearchTreeView.hpp"
...
bool ResearchTreeViewService::apply(const ResearchTreeViewCommand& command) {
...
bool ResearchTreeViewService::erase(std::uint64_t subjectId) {
...
void ResearchTreeViewService::advance(std::uint64_t tick, double delta) {
...
const ResearchTreeViewState* ResearchTreeViewService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ResearchTreeViewState* ResearchTreeViewService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ResearchTreeViewState> ResearchTreeViewService::ordered() const {
    auto result = states_;
...
std::vector<ResearchTreeViewEvent> ResearchTreeViewService::drainEvents() {
...
void ResearchTreeViewService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ResearchTreeViewService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
