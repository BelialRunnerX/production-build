...
#include "presentation/FleetCommandView.hpp"
...
bool FleetCommandViewService::apply(const FleetCommandViewCommand& command) {
...
bool FleetCommandViewService::erase(std::uint64_t subjectId) {
...
void FleetCommandViewService::advance(std::uint64_t tick, double delta) {
...
const FleetCommandViewState* FleetCommandViewService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
FleetCommandViewState* FleetCommandViewService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<FleetCommandViewState> FleetCommandViewService::ordered() const {
    auto result = states_;
...
std::vector<FleetCommandViewEvent> FleetCommandViewService::drainEvents() {
...
void FleetCommandViewService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void FleetCommandViewService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
