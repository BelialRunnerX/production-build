...
#include "presentation/EconomyView.hpp"
...
bool EconomyViewService::apply(const EconomyViewCommand& command) {
...
bool EconomyViewService::erase(std::uint64_t subjectId) {
...
void EconomyViewService::advance(std::uint64_t tick, double delta) {
...
const EconomyViewState* EconomyViewService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
EconomyViewState* EconomyViewService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<EconomyViewState> EconomyViewService::ordered() const {
    auto result = states_;
...
std::vector<EconomyViewEvent> EconomyViewService::drainEvents() {
...
void EconomyViewService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void EconomyViewService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
