// Intended function: Describe camera shots, subjects, timing, transitions, subtitles, and audio cues.
#include "presentation/CinematicSequenceModel.hpp"
...
bool CinematicSequenceModelService::apply(const CinematicSequenceModelCommand& command) {
...
bool CinematicSequenceModelService::erase(std::uint64_t subjectId) {
...
void CinematicSequenceModelService::advance(std::uint64_t tick, double delta) {
...
const CinematicSequenceModelState* CinematicSequenceModelService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CinematicSequenceModelState* CinematicSequenceModelService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CinematicSequenceModelState> CinematicSequenceModelService::ordered() const {
    auto result = states_;
...
std::vector<CinematicSequenceModelEvent> CinematicSequenceModelService::drainEvents() {
...
void CinematicSequenceModelService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CinematicSequenceModelService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
