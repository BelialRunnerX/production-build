// Intended function: Project route capacity, congestion, stock pressure, failures, and service-level breaches.
#include "presentation/LogisticsHeatmapView.hpp"
...
bool LogisticsHeatmapViewService::apply(const LogisticsHeatmapViewCommand& command) {
...
bool LogisticsHeatmapViewService::erase(std::uint64_t subjectId) {
...
void LogisticsHeatmapViewService::advance(std::uint64_t tick, double delta) {
...
const LogisticsHeatmapViewState* LogisticsHeatmapViewService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
LogisticsHeatmapViewState* LogisticsHeatmapViewService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<LogisticsHeatmapViewState> LogisticsHeatmapViewService::ordered() const {
    auto result = states_;
...
std::vector<LogisticsHeatmapViewEvent> LogisticsHeatmapViewService::drainEvents() {
...
void LogisticsHeatmapViewService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void LogisticsHeatmapViewService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
