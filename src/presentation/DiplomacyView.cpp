...
#include "presentation/DiplomacyView.hpp"
...
bool DiplomacyViewService::apply(const DiplomacyViewCommand& command) {
...
bool DiplomacyViewService::erase(std::uint64_t subjectId) {
...
void DiplomacyViewService::advance(std::uint64_t tick, double delta) {
...
const DiplomacyViewState* DiplomacyViewService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DiplomacyViewState* DiplomacyViewService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DiplomacyViewState> DiplomacyViewService::ordered() const {
    auto result = states_;
...
std::vector<DiplomacyViewEvent> DiplomacyViewService::drainEvents() {
...
void DiplomacyViewService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DiplomacyViewService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
