#include "gameplay/CraftingQueue.hpp"
...
bool CraftingQueueService::submit(const CraftingQueueCommand& command) {
...
const CraftingQueueRecord* CraftingQueueService::lookup(std::uint64_t subjectId) const {
    auto it = records_.find(subjectId);
    return it == records_.end() ? nullptr : &it->second;
}

std::vector<CraftingQueueRecord> CraftingQueueService::snapshot() const {
    std::vector<CraftingQueueRecord> out; out.reserve(records_.size());
...
bool CraftingQueueService::remove(std::uint64_t subjectId) { return records_.erase(subjectId) != 0; }
void CraftingQueueService::reset() { records_.clear(); revision_ = 1; }
