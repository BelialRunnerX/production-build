#include "gameplay/UpgradeTree.hpp"
...
bool UpgradeTreeService::submit(const UpgradeTreeCommand& command) {
...
const UpgradeTreeRecord* UpgradeTreeService::lookup(std::uint64_t subjectId) const {
    auto it = records_.find(subjectId);
    return it == records_.end() ? nullptr : &it->second;
}

std::vector<UpgradeTreeRecord> UpgradeTreeService::snapshot() const {
    std::vector<UpgradeTreeRecord> out; out.reserve(records_.size());
...
bool UpgradeTreeService::remove(std::uint64_t subjectId) { return records_.erase(subjectId) != 0; }
void UpgradeTreeService::reset() { records_.clear(); revision_ = 1; }
