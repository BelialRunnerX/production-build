#include "loot/AffixGenerator.hpp"
...
bool AffixGeneratorService::submit(const AffixGeneratorCommand& command) {
...
const AffixGeneratorRecord* AffixGeneratorService::lookup(std::uint64_t subjectId) const {
    auto it = records_.find(subjectId);
    return it == records_.end() ? nullptr : &it->second;
}

std::vector<AffixGeneratorRecord> AffixGeneratorService::snapshot() const {
    std::vector<AffixGeneratorRecord> out; out.reserve(records_.size());
...
bool AffixGeneratorService::remove(std::uint64_t subjectId) { return records_.erase(subjectId) != 0; }
void AffixGeneratorService::reset() { records_.clear(); revision_ = 1; }
