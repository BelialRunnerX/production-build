...
#include "ai/LogisticsDirectorAI.hpp"
...
bool LogisticsDirectorAIService::apply(const LogisticsDirectorAICommand& command) {
...
bool LogisticsDirectorAIService::erase(std::uint64_t subjectId) {
...
void LogisticsDirectorAIService::advance(std::uint64_t tick, double delta) {
...
const LogisticsDirectorAIState* LogisticsDirectorAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
LogisticsDirectorAIState* LogisticsDirectorAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<LogisticsDirectorAIState> LogisticsDirectorAIService::ordered() const {
    auto result = states_;
...
std::vector<LogisticsDirectorAIEvent> LogisticsDirectorAIService::drainEvents() {
...
void LogisticsDirectorAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void LogisticsDirectorAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
