...
#include "ai/ExplorationPlannerAI.hpp"
...
bool ExplorationPlannerAIService::apply(const ExplorationPlannerAICommand& command) {
...
bool ExplorationPlannerAIService::erase(std::uint64_t subjectId) {
...
void ExplorationPlannerAIService::advance(std::uint64_t tick, double delta) {
...
const ExplorationPlannerAIState* ExplorationPlannerAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ExplorationPlannerAIState* ExplorationPlannerAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ExplorationPlannerAIState> ExplorationPlannerAIService::ordered() const {
    auto result = states_;
...
std::vector<ExplorationPlannerAIEvent> ExplorationPlannerAIService::drainEvents() {
...
void ExplorationPlannerAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ExplorationPlannerAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
