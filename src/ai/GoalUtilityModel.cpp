#include "ai/GoalUtilityModel.hpp"
...
bool GoalUtilityModelSystem::apply(const GoalUtilityModelRequest& request) {
...
bool GoalUtilityModelSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const GoalUtilityModelState* GoalUtilityModelSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> GoalUtilityModelSystem::activeIds() const {
...
void GoalUtilityModelSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
