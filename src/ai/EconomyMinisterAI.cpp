...
#include "ai/EconomyMinisterAI.hpp"
...
bool EconomyMinisterAIService::apply(const EconomyMinisterAICommand& command) {
...
bool EconomyMinisterAIService::erase(std::uint64_t subjectId) {
...
void EconomyMinisterAIService::advance(std::uint64_t tick, double delta) {
...
const EconomyMinisterAIState* EconomyMinisterAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
EconomyMinisterAIState* EconomyMinisterAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<EconomyMinisterAIState> EconomyMinisterAIService::ordered() const {
    auto result = states_;
...
std::vector<EconomyMinisterAIEvent> EconomyMinisterAIService::drainEvents() {
...
void EconomyMinisterAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void EconomyMinisterAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
