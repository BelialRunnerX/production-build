...
#include "ai/DiplomaticStrategyAI.hpp"
...
bool DiplomaticStrategyAIService::apply(const DiplomaticStrategyAICommand& command) {
...
bool DiplomaticStrategyAIService::erase(std::uint64_t subjectId) {
...
void DiplomaticStrategyAIService::advance(std::uint64_t tick, double delta) {
...
const DiplomaticStrategyAIState* DiplomaticStrategyAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DiplomaticStrategyAIState* DiplomaticStrategyAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DiplomaticStrategyAIState> DiplomaticStrategyAIService::ordered() const {
    auto result = states_;
...
std::vector<DiplomaticStrategyAIEvent> DiplomaticStrategyAIService::drainEvents() {
...
void DiplomaticStrategyAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DiplomaticStrategyAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
