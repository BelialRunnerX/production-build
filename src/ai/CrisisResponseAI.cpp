...
#include "ai/CrisisResponseAI.hpp"
...
bool CrisisResponseAIService::apply(const CrisisResponseAICommand& command) {
...
bool CrisisResponseAIService::erase(std::uint64_t subjectId) {
...
void CrisisResponseAIService::advance(std::uint64_t tick, double delta) {
...
const CrisisResponseAIState* CrisisResponseAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CrisisResponseAIState* CrisisResponseAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CrisisResponseAIState> CrisisResponseAIService::ordered() const {
    auto result = states_;
...
std::vector<CrisisResponseAIEvent> CrisisResponseAIService::drainEvents() {
...
void CrisisResponseAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CrisisResponseAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
