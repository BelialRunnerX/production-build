// Intended function: Select strategic war goals, theaters, reserves, supply priorities, and de-escalation thresholds.
#include "ai/WarPlannerAI.hpp"
...
bool WarPlannerAIService::apply(const WarPlannerAICommand& command) {
...
bool WarPlannerAIService::erase(std::uint64_t subjectId) {
...
void WarPlannerAIService::advance(std::uint64_t tick, double delta) {
...
const WarPlannerAIState* WarPlannerAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
WarPlannerAIState* WarPlannerAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<WarPlannerAIState> WarPlannerAIService::ordered() const {
    auto result = states_;
...
std::vector<WarPlannerAIEvent> WarPlannerAIService::drainEvents() {
...
void WarPlannerAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void WarPlannerAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
