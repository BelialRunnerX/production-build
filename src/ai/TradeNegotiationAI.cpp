#include "ai/TradeNegotiationAI.hpp"
...
bool TradeNegotiationAISystem::apply(const TradeNegotiationAIRequest& request) {
...
bool TradeNegotiationAISystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const TradeNegotiationAIState* TradeNegotiationAISystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> TradeNegotiationAISystem::activeIds() const {
...
void TradeNegotiationAISystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
