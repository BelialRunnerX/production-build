#include "ai/CreatureBehaviorPlanner.hpp"
...
bool CreatureBehaviorPlannerSystem::apply(const CreatureBehaviorPlannerRequest& request) {
...
bool CreatureBehaviorPlannerSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const CreatureBehaviorPlannerState* CreatureBehaviorPlannerSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> CreatureBehaviorPlannerSystem::activeIds() const {
...
void CreatureBehaviorPlannerSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
