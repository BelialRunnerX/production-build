...
#include "ai/ColonyGovernorAI.hpp"
...
bool ColonyGovernorAIService::apply(const ColonyGovernorAICommand& command) {
...
bool ColonyGovernorAIService::erase(std::uint64_t subjectId) {
...
void ColonyGovernorAIService::advance(std::uint64_t tick, double delta) {
...
const ColonyGovernorAIState* ColonyGovernorAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ColonyGovernorAIState* ColonyGovernorAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ColonyGovernorAIState> ColonyGovernorAIService::ordered() const {
    auto result = states_;
...
std::vector<ColonyGovernorAIEvent> ColonyGovernorAIService::drainEvents() {
...
void ColonyGovernorAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ColonyGovernorAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
