...
#include "ai/SettlementExpansionAI.hpp"
...
bool SettlementExpansionAIService::apply(const SettlementExpansionAICommand& command) {
...
bool SettlementExpansionAIService::erase(std::uint64_t subjectId) {
...
void SettlementExpansionAIService::advance(std::uint64_t tick, double delta) {
...
const SettlementExpansionAIState* SettlementExpansionAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SettlementExpansionAIState* SettlementExpansionAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SettlementExpansionAIState> SettlementExpansionAIService::ordered() const {
    auto result = states_;
...
std::vector<SettlementExpansionAIEvent> SettlementExpansionAIService::drainEvents() {
...
void SettlementExpansionAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SettlementExpansionAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
