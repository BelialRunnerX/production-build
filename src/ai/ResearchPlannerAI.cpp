...
#include "ai/ResearchPlannerAI.hpp"
...
bool ResearchPlannerAIService::apply(const ResearchPlannerAICommand& command) {
...
bool ResearchPlannerAIService::erase(std::uint64_t subjectId) {
...
void ResearchPlannerAIService::advance(std::uint64_t tick, double delta) {
...
const ResearchPlannerAIState* ResearchPlannerAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ResearchPlannerAIState* ResearchPlannerAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ResearchPlannerAIState> ResearchPlannerAIService::ordered() const {
    auto result = states_;
...
std::vector<ResearchPlannerAIEvent> ResearchPlannerAIService::drainEvents() {
...
void ResearchPlannerAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ResearchPlannerAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
