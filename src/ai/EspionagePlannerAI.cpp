...
#include "ai/EspionagePlannerAI.hpp"
...
bool EspionagePlannerAIService::apply(const EspionagePlannerAICommand& command) {
...
bool EspionagePlannerAIService::erase(std::uint64_t subjectId) {
...
void EspionagePlannerAIService::advance(std::uint64_t tick, double delta) {
...
const EspionagePlannerAIState* EspionagePlannerAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
EspionagePlannerAIState* EspionagePlannerAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<EspionagePlannerAIState> EspionagePlannerAIService::ordered() const {
    auto result = states_;
...
std::vector<EspionagePlannerAIEvent> EspionagePlannerAIService::drainEvents() {
...
void EspionagePlannerAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void EspionagePlannerAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
