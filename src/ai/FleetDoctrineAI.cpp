// Intended function: Translate faction doctrine into fleet composition, engagement rules, patrols, and reserves.
#include "ai/FleetDoctrineAI.hpp"
...
bool FleetDoctrineAIService::apply(const FleetDoctrineAICommand& command) {
...
bool FleetDoctrineAIService::erase(std::uint64_t subjectId) {
...
void FleetDoctrineAIService::advance(std::uint64_t tick, double delta) {
...
const FleetDoctrineAIState* FleetDoctrineAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
FleetDoctrineAIState* FleetDoctrineAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<FleetDoctrineAIState> FleetDoctrineAIService::ordered() const {
    auto result = states_;
...
std::vector<FleetDoctrineAIEvent> FleetDoctrineAIService::drainEvents() {
...
void FleetDoctrineAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void FleetDoctrineAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
