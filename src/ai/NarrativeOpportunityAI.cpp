...
#include "ai/NarrativeOpportunityAI.hpp"
...
bool NarrativeOpportunityAIService::apply(const NarrativeOpportunityAICommand& command) {
...
bool NarrativeOpportunityAIService::erase(std::uint64_t subjectId) {
...
void NarrativeOpportunityAIService::advance(std::uint64_t tick, double delta) {
...
const NarrativeOpportunityAIState* NarrativeOpportunityAIService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
NarrativeOpportunityAIState* NarrativeOpportunityAIService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<NarrativeOpportunityAIState> NarrativeOpportunityAIService::ordered() const {
    auto result = states_;
...
std::vector<NarrativeOpportunityAIEvent> NarrativeOpportunityAIService::drainEvents() {
...
void NarrativeOpportunityAIService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void NarrativeOpportunityAIService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
