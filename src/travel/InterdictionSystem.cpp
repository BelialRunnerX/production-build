#include "travel/InterdictionSystem.hpp"
...
bool InterdictionSystemSystem::apply(const InterdictionSystemRequest& request) {
...
bool InterdictionSystemSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const InterdictionSystemState* InterdictionSystemSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> InterdictionSystemSystem::activeIds() const {
...
void InterdictionSystemSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
