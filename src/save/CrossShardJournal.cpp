#include "save/CrossShardJournal.hpp"
...
bool CrossShardJournalSystem::apply(const CrossShardJournalRequest& request) {
...
bool CrossShardJournalSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const CrossShardJournalState* CrossShardJournalSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> CrossShardJournalSystem::activeIds() const {
...
void CrossShardJournalSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
