// Intended function: Measure habitat connectivity loss from infrastructure and restoration corridors.
#include "ecology/HabitatFragmentation.hpp"
...
bool HabitatFragmentationService::apply(const HabitatFragmentationCommand& command) {
...
bool HabitatFragmentationService::erase(std::uint64_t subjectId) {
...
void HabitatFragmentationService::advance(std::uint64_t tick, double delta) {
...
const HabitatFragmentationState* HabitatFragmentationService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
HabitatFragmentationState* HabitatFragmentationService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<HabitatFragmentationState> HabitatFragmentationService::ordered() const {
    auto result = states_;
...
std::vector<HabitatFragmentationEvent> HabitatFragmentationService::drainEvents() {
...
void HabitatFragmentationService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void HabitatFragmentationService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
