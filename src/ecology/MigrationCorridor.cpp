...
#include "ecology/MigrationCorridor.hpp"
...
bool MigrationCorridorService::apply(const MigrationCorridorCommand& command) {
...
bool MigrationCorridorService::erase(std::uint64_t subjectId) {
...
void MigrationCorridorService::advance(std::uint64_t tick, double delta) {
...
const MigrationCorridorState* MigrationCorridorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
MigrationCorridorState* MigrationCorridorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<MigrationCorridorState> MigrationCorridorService::ordered() const {
    auto result = states_;
...
std::vector<MigrationCorridorEvent> MigrationCorridorService::drainEvents() {
...
void MigrationCorridorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void MigrationCorridorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
