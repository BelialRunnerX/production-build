...
#include "ecology/AquaticEcology.hpp"
...
bool AquaticEcologyService::apply(const AquaticEcologyCommand& command) {
...
bool AquaticEcologyService::erase(std::uint64_t subjectId) {
...
void AquaticEcologyService::advance(std::uint64_t tick, double delta) {
...
const AquaticEcologyState* AquaticEcologyService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
AquaticEcologyState* AquaticEcologyService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<AquaticEcologyState> AquaticEcologyService::ordered() const {
    auto result = states_;
...
std::vector<AquaticEcologyEvent> AquaticEcologyService::drainEvents() {
...
void AquaticEcologyService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void AquaticEcologyService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
