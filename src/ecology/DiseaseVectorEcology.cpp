...
#include "ecology/DiseaseVectorEcology.hpp"
...
bool DiseaseVectorEcologyService::apply(const DiseaseVectorEcologyCommand& command) {
...
bool DiseaseVectorEcologyService::erase(std::uint64_t subjectId) {
...
void DiseaseVectorEcologyService::advance(std::uint64_t tick, double delta) {
...
const DiseaseVectorEcologyState* DiseaseVectorEcologyService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DiseaseVectorEcologyState* DiseaseVectorEcologyService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DiseaseVectorEcologyState> DiseaseVectorEcologyService::ordered() const {
    auto result = states_;
...
std::vector<DiseaseVectorEcologyEvent> DiseaseVectorEcologyService::drainEvents() {
...
void DiseaseVectorEcologyService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DiseaseVectorEcologyService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
