...
#include "ecology/PredatorPreyModel.hpp"
...
bool PredatorPreyModelService::apply(const PredatorPreyModelCommand& command) {
...
bool PredatorPreyModelService::erase(std::uint64_t subjectId) {
...
void PredatorPreyModelService::advance(std::uint64_t tick, double delta) {
...
const PredatorPreyModelState* PredatorPreyModelService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PredatorPreyModelState* PredatorPreyModelService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PredatorPreyModelState> PredatorPreyModelService::ordered() const {
    auto result = states_;
...
std::vector<PredatorPreyModelEvent> PredatorPreyModelService::drainEvents() {
...
void PredatorPreyModelService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PredatorPreyModelService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
