// Intended function: Track invasive spread, ecological competition, control efforts, and biome transformation.
#include "ecology/InvasiveSpecies.hpp"
...
bool InvasiveSpeciesService::apply(const InvasiveSpeciesCommand& command) {
...
bool InvasiveSpeciesService::erase(std::uint64_t subjectId) {
...
void InvasiveSpeciesService::advance(std::uint64_t tick, double delta) {
...
const InvasiveSpeciesState* InvasiveSpeciesService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
InvasiveSpeciesState* InvasiveSpeciesService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<InvasiveSpeciesState> InvasiveSpeciesService::ordered() const {
    auto result = states_;
...
std::vector<InvasiveSpeciesEvent> InvasiveSpeciesService::drainEvents() {
...
void InvasiveSpeciesService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void InvasiveSpeciesService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
