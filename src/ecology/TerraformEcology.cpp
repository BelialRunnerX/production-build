// Intended function: Track ecological adaptation pressure while atmosphere, hydrology, and climate are modified.
#include "ecology/TerraformEcology.hpp"
...
bool TerraformEcologyService::apply(const TerraformEcologyCommand& command) {
...
bool TerraformEcologyService::erase(std::uint64_t subjectId) {
...
void TerraformEcologyService::advance(std::uint64_t tick, double delta) {
...
const TerraformEcologyState* TerraformEcologyService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
TerraformEcologyState* TerraformEcologyService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<TerraformEcologyState> TerraformEcologyService::ordered() const {
    auto result = states_;
...
std::vector<TerraformEcologyEvent> TerraformEcologyService::drainEvents() {
...
void TerraformEcologyService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void TerraformEcologyService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
