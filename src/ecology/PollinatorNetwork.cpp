...
#include "ecology/PollinatorNetwork.hpp"
...
bool PollinatorNetworkService::apply(const PollinatorNetworkCommand& command) {
...
bool PollinatorNetworkService::erase(std::uint64_t subjectId) {
...
void PollinatorNetworkService::advance(std::uint64_t tick, double delta) {
...
const PollinatorNetworkState* PollinatorNetworkService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PollinatorNetworkState* PollinatorNetworkService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PollinatorNetworkState> PollinatorNetworkService::ordered() const {
    auto result = states_;
...
std::vector<PollinatorNetworkEvent> PollinatorNetworkService::drainEvents() {
...
void PollinatorNetworkService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PollinatorNetworkService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
