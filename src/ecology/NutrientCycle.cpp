...
#include "ecology/NutrientCycle.hpp"
...
bool NutrientCycleService::apply(const NutrientCycleCommand& command) {
...
bool NutrientCycleService::erase(std::uint64_t subjectId) {
...
void NutrientCycleService::advance(std::uint64_t tick, double delta) {
...
const NutrientCycleState* NutrientCycleService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
NutrientCycleState* NutrientCycleService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<NutrientCycleState> NutrientCycleService::ordered() const {
    auto result = states_;
...
std::vector<NutrientCycleEvent> NutrientCycleService::drainEvents() {
...
void NutrientCycleService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void NutrientCycleService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
