...
#include "ecology/ConservationPolicy.hpp"
...
bool ConservationPolicyService::apply(const ConservationPolicyCommand& command) {
...
bool ConservationPolicyService::erase(std::uint64_t subjectId) {
...
void ConservationPolicyService::advance(std::uint64_t tick, double delta) {
...
const ConservationPolicyState* ConservationPolicyService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ConservationPolicyState* ConservationPolicyService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ConservationPolicyState> ConservationPolicyService::ordered() const {
    auto result = states_;
...
std::vector<ConservationPolicyEvent> ConservationPolicyService::drainEvents() {
...
void ConservationPolicyService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ConservationPolicyService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
