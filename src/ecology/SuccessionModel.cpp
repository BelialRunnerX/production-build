// Intended function: Track ecological succession after fire, mining, terraforming, abandonment, or disaster.
#include "ecology/SuccessionModel.hpp"
...
bool SuccessionModelService::apply(const SuccessionModelCommand& command) {
...
bool SuccessionModelService::erase(std::uint64_t subjectId) {
...
void SuccessionModelService::advance(std::uint64_t tick, double delta) {
...
const SuccessionModelState* SuccessionModelService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SuccessionModelState* SuccessionModelService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SuccessionModelState> SuccessionModelService::ordered() const {
    auto result = states_;
...
std::vector<SuccessionModelEvent> SuccessionModelService::drainEvents() {
...
void SuccessionModelService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SuccessionModelService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
