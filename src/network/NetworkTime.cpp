...
#include "network/NetworkTime.hpp"
...
bool NetworkTimeService::apply(const NetworkTimeCommand& command) {
...
bool NetworkTimeService::erase(std::uint64_t subjectId) {
...
void NetworkTimeService::advance(std::uint64_t tick, double delta) {
...
const NetworkTimeState* NetworkTimeService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
NetworkTimeState* NetworkTimeService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<NetworkTimeState> NetworkTimeService::ordered() const {
    auto result = states_;
...
std::vector<NetworkTimeEvent> NetworkTimeService::drainEvents() {
...
void NetworkTimeService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void NetworkTimeService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
