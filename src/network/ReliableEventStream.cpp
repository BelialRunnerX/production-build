...
#include "network/ReliableEventStream.hpp"
...
bool ReliableEventStreamService::apply(const ReliableEventStreamCommand& command) {
...
bool ReliableEventStreamService::erase(std::uint64_t subjectId) {
...
void ReliableEventStreamService::advance(std::uint64_t tick, double delta) {
...
const ReliableEventStreamState* ReliableEventStreamService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ReliableEventStreamState* ReliableEventStreamService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ReliableEventStreamState> ReliableEventStreamService::ordered() const {
    auto result = states_;
...
std::vector<ReliableEventStreamEvent> ReliableEventStreamService::drainEvents() {
...
void ReliableEventStreamService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ReliableEventStreamService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
