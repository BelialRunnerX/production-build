...
#include "network/NetworkSecurityPolicy.hpp"
...
bool NetworkSecurityPolicyService::apply(const NetworkSecurityPolicyCommand& command) {
...
bool NetworkSecurityPolicyService::erase(std::uint64_t subjectId) {
...
void NetworkSecurityPolicyService::advance(std::uint64_t tick, double delta) {
...
const NetworkSecurityPolicyState* NetworkSecurityPolicyService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
NetworkSecurityPolicyState* NetworkSecurityPolicyService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<NetworkSecurityPolicyState> NetworkSecurityPolicyService::ordered() const {
    auto result = states_;
...
std::vector<NetworkSecurityPolicyEvent> NetworkSecurityPolicyService::drainEvents() {
...
void NetworkSecurityPolicyService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void NetworkSecurityPolicyService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
