...
#include "network/DesyncDiagnostics.hpp"
...
bool DesyncDiagnosticsService::apply(const DesyncDiagnosticsCommand& command) {
...
bool DesyncDiagnosticsService::erase(std::uint64_t subjectId) {
...
void DesyncDiagnosticsService::advance(std::uint64_t tick, double delta) {
...
const DesyncDiagnosticsState* DesyncDiagnosticsService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DesyncDiagnosticsState* DesyncDiagnosticsService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DesyncDiagnosticsState> DesyncDiagnosticsService::ordered() const {
    auto result = states_;
...
std::vector<DesyncDiagnosticsEvent> DesyncDiagnosticsService::drainEvents() {
...
void DesyncDiagnosticsService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DesyncDiagnosticsService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
