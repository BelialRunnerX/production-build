...
#include "network/ClientPrediction.hpp"
...
bool ClientPredictionService::apply(const ClientPredictionCommand& command) {
...
bool ClientPredictionService::erase(std::uint64_t subjectId) {
...
void ClientPredictionService::advance(std::uint64_t tick, double delta) {
...
const ClientPredictionState* ClientPredictionService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ClientPredictionState* ClientPredictionService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ClientPredictionState> ClientPredictionService::ordered() const {
    auto result = states_;
...
std::vector<ClientPredictionEvent> ClientPredictionService::drainEvents() {
...
void ClientPredictionService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ClientPredictionService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
