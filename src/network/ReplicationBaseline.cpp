// Intended function: Track acknowledged durable baselines for compact state delta transmission.
#include "network/ReplicationBaseline.hpp"
...
bool ReplicationBaselineService::apply(const ReplicationBaselineCommand& command) {
...
bool ReplicationBaselineService::erase(std::uint64_t subjectId) {
...
void ReplicationBaselineService::advance(std::uint64_t tick, double delta) {
...
const ReplicationBaselineState* ReplicationBaselineService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ReplicationBaselineState* ReplicationBaselineService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ReplicationBaselineState> ReplicationBaselineService::ordered() const {
    auto result = states_;
...
std::vector<ReplicationBaselineEvent> ReplicationBaselineService::drainEvents() {
...
void ReplicationBaselineService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ReplicationBaselineService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
