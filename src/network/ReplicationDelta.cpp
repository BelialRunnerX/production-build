#include "network/ReplicationDelta.hpp"
...
bool ReplicationDeltaSystem::apply(const ReplicationDeltaRequest& request) {
...
bool ReplicationDeltaSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const ReplicationDeltaState* ReplicationDeltaSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> ReplicationDeltaSystem::activeIds() const {
...
void ReplicationDeltaSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
