...
#include "network/SessionMigration.hpp"
...
bool SessionMigrationService::apply(const SessionMigrationCommand& command) {
...
bool SessionMigrationService::erase(std::uint64_t subjectId) {
...
void SessionMigrationService::advance(std::uint64_t tick, double delta) {
...
const SessionMigrationState* SessionMigrationService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SessionMigrationState* SessionMigrationService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SessionMigrationState> SessionMigrationService::ordered() const {
    auto result = states_;
...
std::vector<SessionMigrationEvent> SessionMigrationService::drainEvents() {
...
void SessionMigrationService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SessionMigrationService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
