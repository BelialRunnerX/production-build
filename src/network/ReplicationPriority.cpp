// Intended function: Score durable state for replication from distance, relevance, ownership, and urgency.
#include "network/ReplicationPriority.hpp"
...
bool ReplicationPriorityService::apply(const ReplicationPriorityCommand& command) {
...
bool ReplicationPriorityService::erase(std::uint64_t subjectId) {
...
void ReplicationPriorityService::advance(std::uint64_t tick, double delta) {
...
const ReplicationPriorityState* ReplicationPriorityService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ReplicationPriorityState* ReplicationPriorityService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ReplicationPriorityState> ReplicationPriorityService::ordered() const {
    auto result = states_;
...
std::vector<ReplicationPriorityEvent> ReplicationPriorityService::drainEvents() {
...
void ReplicationPriorityService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ReplicationPriorityService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
