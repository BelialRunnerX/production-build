// Intended function: Track latest-wins high-rate presentation state for future networking.
#include "network/UnreliableStateStream.hpp"
...
bool UnreliableStateStreamService::apply(const UnreliableStateStreamCommand& command) {
...
bool UnreliableStateStreamService::erase(std::uint64_t subjectId) {
...
void UnreliableStateStreamService::advance(std::uint64_t tick, double delta) {
...
const UnreliableStateStreamState* UnreliableStateStreamService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
UnreliableStateStreamState* UnreliableStateStreamService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<UnreliableStateStreamState> UnreliableStateStreamService::ordered() const {
    auto result = states_;
...
std::vector<UnreliableStateStreamEvent> UnreliableStateStreamService::drainEvents() {
...
void UnreliableStateStreamService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void UnreliableStateStreamService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
