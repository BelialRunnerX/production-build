// Intended function: Negotiate future multiplayer protocol features, schema versions, and optional capabilities.
#include "network/CapabilityNegotiation.hpp"
...
bool CapabilityNegotiationService::apply(const CapabilityNegotiationCommand& command) {
...
bool CapabilityNegotiationService::erase(std::uint64_t subjectId) {
...
void CapabilityNegotiationService::advance(std::uint64_t tick, double delta) {
...
const CapabilityNegotiationState* CapabilityNegotiationService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CapabilityNegotiationState* CapabilityNegotiationService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CapabilityNegotiationState> CapabilityNegotiationService::ordered() const {
    auto result = states_;
...
std::vector<CapabilityNegotiationEvent> CapabilityNegotiationService::drainEvents() {
...
void CapabilityNegotiationService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CapabilityNegotiationService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
