...
#include "network/ServerAuthority.hpp"
...
bool ServerAuthorityService::apply(const ServerAuthorityCommand& command) {
...
bool ServerAuthorityService::erase(std::uint64_t subjectId) {
...
void ServerAuthorityService::advance(std::uint64_t tick, double delta) {
...
const ServerAuthorityState* ServerAuthorityService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ServerAuthorityState* ServerAuthorityService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ServerAuthorityState> ServerAuthorityService::ordered() const {
    auto result = states_;
...
std::vector<ServerAuthorityEvent> ServerAuthorityService::drainEvents() {
...
void ServerAuthorityService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ServerAuthorityService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
