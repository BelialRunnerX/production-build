...
#include "audio/ImpactAudioModel.hpp"
...
bool ImpactAudioModelService::apply(const ImpactAudioModelCommand& command) {
...
bool ImpactAudioModelService::erase(std::uint64_t subjectId) {
...
void ImpactAudioModelService::advance(std::uint64_t tick, double delta) {
...
const ImpactAudioModelState* ImpactAudioModelService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ImpactAudioModelState* ImpactAudioModelService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ImpactAudioModelState> ImpactAudioModelService::ordered() const {
    auto result = states_;
...
std::vector<ImpactAudioModelEvent> ImpactAudioModelService::drainEvents() {
...
void ImpactAudioModelService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ImpactAudioModelService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
