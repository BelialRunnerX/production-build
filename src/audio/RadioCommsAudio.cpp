...
#include "audio/RadioCommsAudio.hpp"
...
bool RadioCommsAudioService::apply(const RadioCommsAudioCommand& command) {
...
bool RadioCommsAudioService::erase(std::uint64_t subjectId) {
...
void RadioCommsAudioService::advance(std::uint64_t tick, double delta) {
...
const RadioCommsAudioState* RadioCommsAudioService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
RadioCommsAudioState* RadioCommsAudioService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<RadioCommsAudioState> RadioCommsAudioService::ordered() const {
    auto result = states_;
...
std::vector<RadioCommsAudioEvent> RadioCommsAudioService::drainEvents() {
...
void RadioCommsAudioService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void RadioCommsAudioService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
