// Intended function: Track bounded interior/exterior/reverb zones and transitions for backend-neutral audio.
#include "audio/SpatialAudioZones.hpp"
...
bool SpatialAudioZonesService::apply(const SpatialAudioZonesCommand& command) {
...
bool SpatialAudioZonesService::erase(std::uint64_t subjectId) {
...
void SpatialAudioZonesService::advance(std::uint64_t tick, double delta) {
...
const SpatialAudioZonesState* SpatialAudioZonesService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SpatialAudioZonesState* SpatialAudioZonesService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SpatialAudioZonesState> SpatialAudioZonesService::ordered() const {
    auto result = states_;
...
std::vector<SpatialAudioZonesEvent> SpatialAudioZonesService::drainEvents() {
...
void SpatialAudioZonesService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SpatialAudioZonesService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
