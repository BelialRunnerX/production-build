// Intended function: Emit optional directional/nonvisual cues for hazards, objectives, targets, and interactions.
#include "audio/AccessibilityAudioCues.hpp"
...
bool AccessibilityAudioCuesService::apply(const AccessibilityAudioCuesCommand& command) {
...
bool AccessibilityAudioCuesService::erase(std::uint64_t subjectId) {
...
void AccessibilityAudioCuesService::advance(std::uint64_t tick, double delta) {
...
const AccessibilityAudioCuesState* AccessibilityAudioCuesService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
AccessibilityAudioCuesState* AccessibilityAudioCuesService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<AccessibilityAudioCuesState> AccessibilityAudioCuesService::ordered() const {
    auto result = states_;
...
std::vector<AccessibilityAudioCuesEvent> AccessibilityAudioCuesService::drainEvents() {
...
void AccessibilityAudioCuesService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void AccessibilityAudioCuesService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
