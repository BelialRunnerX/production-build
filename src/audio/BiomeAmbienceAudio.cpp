...
#include "audio/BiomeAmbienceAudio.hpp"
...
bool BiomeAmbienceAudioService::apply(const BiomeAmbienceAudioCommand& command) {
...
bool BiomeAmbienceAudioService::erase(std::uint64_t subjectId) {
...
void BiomeAmbienceAudioService::advance(std::uint64_t tick, double delta) {
...
const BiomeAmbienceAudioState* BiomeAmbienceAudioService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
BiomeAmbienceAudioState* BiomeAmbienceAudioService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<BiomeAmbienceAudioState> BiomeAmbienceAudioService::ordered() const {
    auto result = states_;
...
std::vector<BiomeAmbienceAudioEvent> BiomeAmbienceAudioService::drainEvents() {
...
void BiomeAmbienceAudioService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void BiomeAmbienceAudioService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
