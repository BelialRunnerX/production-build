...
#include "audio/DestructionAudioModel.hpp"
...
bool DestructionAudioModelService::apply(const DestructionAudioModelCommand& command) {
...
bool DestructionAudioModelService::erase(std::uint64_t subjectId) {
...
void DestructionAudioModelService::advance(std::uint64_t tick, double delta) {
...
const DestructionAudioModelState* DestructionAudioModelService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DestructionAudioModelState* DestructionAudioModelService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DestructionAudioModelState> DestructionAudioModelService::ordered() const {
    auto result = states_;
...
std::vector<DestructionAudioModelEvent> DestructionAudioModelService::drainEvents() {
...
void DestructionAudioModelService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DestructionAudioModelService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
