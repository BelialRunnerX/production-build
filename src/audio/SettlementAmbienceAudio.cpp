...
#include "audio/SettlementAmbienceAudio.hpp"
...
bool SettlementAmbienceAudioService::apply(const SettlementAmbienceAudioCommand& command) {
...
bool SettlementAmbienceAudioService::erase(std::uint64_t subjectId) {
...
void SettlementAmbienceAudioService::advance(std::uint64_t tick, double delta) {
...
const SettlementAmbienceAudioState* SettlementAmbienceAudioService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SettlementAmbienceAudioState* SettlementAmbienceAudioService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SettlementAmbienceAudioState> SettlementAmbienceAudioService::ordered() const {
    auto result = states_;
...
std::vector<SettlementAmbienceAudioEvent> SettlementAmbienceAudioService::drainEvents() {
...
void SettlementAmbienceAudioService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SettlementAmbienceAudioService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
