// Intended function: Prioritize dialogue, warnings, squad calls, radio, and ambient voice without overlap spam.
#include "audio/VoicePriorityMixer.hpp"
...
bool VoicePriorityMixerService::apply(const VoicePriorityMixerCommand& command) {
...
bool VoicePriorityMixerService::erase(std::uint64_t subjectId) {
...
void VoicePriorityMixerService::advance(std::uint64_t tick, double delta) {
...
const VoicePriorityMixerState* VoicePriorityMixerService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
VoicePriorityMixerState* VoicePriorityMixerService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<VoicePriorityMixerState> VoicePriorityMixerService::ordered() const {
    auto result = states_;
...
std::vector<VoicePriorityMixerEvent> VoicePriorityMixerService::drainEvents() {
...
void VoicePriorityMixerService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void VoicePriorityMixerService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
