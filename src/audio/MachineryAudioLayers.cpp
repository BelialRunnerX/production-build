// Intended function: Aggregate machine load, faults, power, and throughput into scalable machinery audio layers.
#include "audio/MachineryAudioLayers.hpp"
...
bool MachineryAudioLayersService::apply(const MachineryAudioLayersCommand& command) {
...
bool MachineryAudioLayersService::erase(std::uint64_t subjectId) {
...
void MachineryAudioLayersService::advance(std::uint64_t tick, double delta) {
...
const MachineryAudioLayersState* MachineryAudioLayersService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
MachineryAudioLayersState* MachineryAudioLayersService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<MachineryAudioLayersState> MachineryAudioLayersService::ordered() const {
    auto result = states_;
...
std::vector<MachineryAudioLayersEvent> MachineryAudioLayersService::drainEvents() {
...
void MachineryAudioLayersService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void MachineryAudioLayersService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
