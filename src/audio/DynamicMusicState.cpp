...
#include "audio/DynamicMusicState.hpp"
...
bool DynamicMusicStateService::apply(const DynamicMusicStateCommand& command) {
...
bool DynamicMusicStateService::erase(std::uint64_t subjectId) {
...
void DynamicMusicStateService::advance(std::uint64_t tick, double delta) {
...
const DynamicMusicStateState* DynamicMusicStateService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DynamicMusicStateState* DynamicMusicStateService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DynamicMusicStateState> DynamicMusicStateService::ordered() const {
    auto result = states_;
...
std::vector<DynamicMusicStateEvent> DynamicMusicStateService::drainEvents() {
...
void DynamicMusicStateService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DynamicMusicStateService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
