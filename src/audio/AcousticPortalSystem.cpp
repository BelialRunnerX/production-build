...
#include "audio/AcousticPortalSystem.hpp"
...
bool AcousticPortalSystemService::apply(const AcousticPortalSystemCommand& command) {
...
bool AcousticPortalSystemService::erase(std::uint64_t subjectId) {
...
void AcousticPortalSystemService::advance(std::uint64_t tick, double delta) {
...
const AcousticPortalSystemState* AcousticPortalSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
AcousticPortalSystemState* AcousticPortalSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<AcousticPortalSystemState> AcousticPortalSystemService::ordered() const {
    auto result = states_;
...
std::vector<AcousticPortalSystemEvent> AcousticPortalSystemService::drainEvents() {
...
void AcousticPortalSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void AcousticPortalSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
