...
#include "audio/ShipInteriorAudio.hpp"
...
bool ShipInteriorAudioService::apply(const ShipInteriorAudioCommand& command) {
...
bool ShipInteriorAudioService::erase(std::uint64_t subjectId) {
...
void ShipInteriorAudioService::advance(std::uint64_t tick, double delta) {
...
const ShipInteriorAudioState* ShipInteriorAudioService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ShipInteriorAudioState* ShipInteriorAudioService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ShipInteriorAudioState> ShipInteriorAudioService::ordered() const {
    auto result = states_;
...
std::vector<ShipInteriorAudioEvent> ShipInteriorAudioService::drainEvents() {
...
void ShipInteriorAudioService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ShipInteriorAudioService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
