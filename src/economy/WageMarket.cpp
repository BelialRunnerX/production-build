// Intended function: Track labor demand, skill scarcity, wage offers, unemployment pressure, and wage drift.
#include "economy/WageMarket.hpp"
...
bool WageMarketService::apply(const WageMarketCommand& command) {
...
bool WageMarketService::erase(std::uint64_t subjectId) {
...
void WageMarketService::advance(std::uint64_t tick, double delta) {
...
const WageMarketState* WageMarketService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
WageMarketState* WageMarketService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<WageMarketState> WageMarketService::ordered() const {
    auto result = states_;
...
std::vector<WageMarketEvent> WageMarketService::drainEvents() {
...
void WageMarketService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void WageMarketService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
