...
#include "economy/CreditMarket.hpp"
...
bool CreditMarketService::apply(const CreditMarketCommand& command) {
...
bool CreditMarketService::erase(std::uint64_t subjectId) {
...
void CreditMarketService::advance(std::uint64_t tick, double delta) {
...
const CreditMarketState* CreditMarketService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CreditMarketState* CreditMarketService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CreditMarketState> CreditMarketService::ordered() const {
    auto result = states_;
...
std::vector<CreditMarketEvent> CreditMarketService::drainEvents() {
...
void CreditMarketService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CreditMarketService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
