...
#include "economy/PriceIndex.hpp"
...
bool PriceIndexService::apply(const PriceIndexCommand& command) {
...
bool PriceIndexService::erase(std::uint64_t subjectId) {
...
void PriceIndexService::advance(std::uint64_t tick, double delta) {
...
const PriceIndexState* PriceIndexService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PriceIndexState* PriceIndexService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PriceIndexState> PriceIndexService::ordered() const {
    auto result = states_;
...
std::vector<PriceIndexEvent> PriceIndexService::drainEvents() {
...
void PriceIndexService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PriceIndexService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
