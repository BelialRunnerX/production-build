...
#include "economy/EconomicShock.hpp"
...
bool EconomicShockService::apply(const EconomicShockCommand& command) {
...
bool EconomicShockService::erase(std::uint64_t subjectId) {
...
void EconomicShockService::advance(std::uint64_t tick, double delta) {
...
const EconomicShockState* EconomicShockService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
EconomicShockState* EconomicShockService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<EconomicShockState> EconomicShockService::ordered() const {
    auto result = states_;
...
std::vector<EconomicShockEvent> EconomicShockService::drainEvents() {
...
void EconomicShockService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void EconomicShockService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
