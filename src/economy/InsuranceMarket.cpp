...
#include "economy/InsuranceMarket.hpp"
...
bool InsuranceMarketService::apply(const InsuranceMarketCommand& command) {
...
bool InsuranceMarketService::erase(std::uint64_t subjectId) {
...
void InsuranceMarketService::advance(std::uint64_t tick, double delta) {
...
const InsuranceMarketState* InsuranceMarketService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
InsuranceMarketState* InsuranceMarketService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<InsuranceMarketState> InsuranceMarketService::ordered() const {
    auto result = states_;
...
std::vector<InsuranceMarketEvent> InsuranceMarketService::drainEvents() {
...
void InsuranceMarketService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void InsuranceMarketService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
