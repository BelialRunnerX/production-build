// Intended function: Track production/consumption subsidies, eligibility, budget draw, and market distortion.
#include "economy/SubsidySystem.hpp"
...
bool SubsidySystemService::apply(const SubsidySystemCommand& command) {
...
bool SubsidySystemService::erase(std::uint64_t subjectId) {
...
void SubsidySystemService::advance(std::uint64_t tick, double delta) {
...
const SubsidySystemState* SubsidySystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SubsidySystemState* SubsidySystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SubsidySystemState> SubsidySystemService::ordered() const {
    auto result = states_;
...
std::vector<SubsidySystemEvent> SubsidySystemService::drainEvents() {
...
void SubsidySystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SubsidySystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
