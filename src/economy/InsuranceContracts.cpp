#include "economy/InsuranceContracts.hpp"
...
bool InsuranceContractsSystem::apply(const InsuranceContractsRequest& request) {
...
bool InsuranceContractsSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const InsuranceContractsState* InsuranceContractsSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> InsuranceContractsSystem::activeIds() const {
...
void InsuranceContractsSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
