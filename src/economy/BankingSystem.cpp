// Intended function: Track deposits, reserves, withdrawals, liquidity pressure, and bounded credit creation.
...
bool BankingSystemService::apply(const BankingSystemCommand& command) {
...
bool BankingSystemService::erase(std::uint64_t subjectId) {
...
void BankingSystemService::advance(std::uint64_t tick, double delta) {
...
const BankingSystemState* BankingSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
BankingSystemState* BankingSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<BankingSystemState> BankingSystemService::ordered() const {
    auto result = states_;
...
std::vector<BankingSystemEvent> BankingSystemService::drainEvents() {
...
void BankingSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void BankingSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
