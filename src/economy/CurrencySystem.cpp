// Intended function: Track currencies, reserves, exchange pressure, convertibility, and faction monetary policy.
#include "economy/CurrencySystem.hpp"
...
bool CurrencySystemService::apply(const CurrencySystemCommand& command) {
...
bool CurrencySystemService::erase(std::uint64_t subjectId) {
...
void CurrencySystemService::advance(std::uint64_t tick, double delta) {
...
const CurrencySystemState* CurrencySystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CurrencySystemState* CurrencySystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CurrencySystemState> CurrencySystemService::ordered() const {
    auto result = states_;
...
std::vector<CurrencySystemEvent> CurrencySystemService::drainEvents() {
...
void CurrencySystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CurrencySystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
