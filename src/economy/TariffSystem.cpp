...
#include "economy/TariffSystem.hpp"
...
bool TariffSystemService::apply(const TariffSystemCommand& command) {
...
bool TariffSystemService::erase(std::uint64_t subjectId) {
...
void TariffSystemService::advance(std::uint64_t tick, double delta) {
...
const TariffSystemState* TariffSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
TariffSystemState* TariffSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<TariffSystemState> TariffSystemService::ordered() const {
    auto result = states_;
...
std::vector<TariffSystemEvent> TariffSystemService::drainEvents() {
...
void TariffSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void TariffSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
