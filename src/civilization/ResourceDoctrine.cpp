...
#include "civilization/ResourceDoctrine.hpp"
...
bool ResourceDoctrineService::apply(const ResourceDoctrineCommand& command) {
...
bool ResourceDoctrineService::erase(std::uint64_t subjectId) {
...
void ResourceDoctrineService::advance(std::uint64_t tick, double delta) {
...
const ResourceDoctrineState* ResourceDoctrineService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ResourceDoctrineState* ResourceDoctrineService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ResourceDoctrineState> ResourceDoctrineService::ordered() const {
    auto result = states_;
...
std::vector<ResourceDoctrineEvent> ResourceDoctrineService::drainEvents() {
...
void ResourceDoctrineService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ResourceDoctrineService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
