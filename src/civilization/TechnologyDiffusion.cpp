...
#include "civilization/TechnologyDiffusion.hpp"
...
bool TechnologyDiffusionService::apply(const TechnologyDiffusionCommand& command) {
...
bool TechnologyDiffusionService::erase(std::uint64_t subjectId) {
...
void TechnologyDiffusionService::advance(std::uint64_t tick, double delta) {
...
const TechnologyDiffusionState* TechnologyDiffusionService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
TechnologyDiffusionState* TechnologyDiffusionService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<TechnologyDiffusionState> TechnologyDiffusionService::ordered() const {
    auto result = states_;
...
std::vector<TechnologyDiffusionEvent> TechnologyDiffusionService::drainEvents() {
...
void TechnologyDiffusionService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void TechnologyDiffusionService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
