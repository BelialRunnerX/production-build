...
#include "civilization/DemographicPressure.hpp"
...
bool DemographicPressureService::apply(const DemographicPressureCommand& command) {
...
bool DemographicPressureService::erase(std::uint64_t subjectId) {
...
void DemographicPressureService::advance(std::uint64_t tick, double delta) {
...
const DemographicPressureState* DemographicPressureService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DemographicPressureState* DemographicPressureService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DemographicPressureState> DemographicPressureService::ordered() const {
    auto result = states_;
...
std::vector<DemographicPressureEvent> DemographicPressureService::drainEvents() {
...
void DemographicPressureService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DemographicPressureService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
