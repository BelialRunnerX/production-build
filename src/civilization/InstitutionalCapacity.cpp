...
#include "civilization/InstitutionalCapacity.hpp"
...
bool InstitutionalCapacityService::apply(const InstitutionalCapacityCommand& command) {
...
bool InstitutionalCapacityService::erase(std::uint64_t subjectId) {
...
void InstitutionalCapacityService::advance(std::uint64_t tick, double delta) {
...
const InstitutionalCapacityState* InstitutionalCapacityService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
InstitutionalCapacityState* InstitutionalCapacityService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<InstitutionalCapacityState> InstitutionalCapacityService::ordered() const {
    auto result = states_;
...
std::vector<InstitutionalCapacityEvent> InstitutionalCapacityService::drainEvents() {
...
void InstitutionalCapacityService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void InstitutionalCapacityService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
