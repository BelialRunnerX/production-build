...
#include "civilization/CivilizationCohesion.hpp"
...
bool CivilizationCohesionService::apply(const CivilizationCohesionCommand& command) {
...
bool CivilizationCohesionService::erase(std::uint64_t subjectId) {
...
void CivilizationCohesionService::advance(std::uint64_t tick, double delta) {
...
const CivilizationCohesionState* CivilizationCohesionService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CivilizationCohesionState* CivilizationCohesionService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CivilizationCohesionState> CivilizationCohesionService::ordered() const {
    auto result = states_;
...
std::vector<CivilizationCohesionEvent> CivilizationCohesionService::drainEvents() {
...
void CivilizationCohesionService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CivilizationCohesionService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
