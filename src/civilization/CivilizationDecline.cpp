...
#include "civilization/CivilizationDecline.hpp"
...
bool CivilizationDeclineService::apply(const CivilizationDeclineCommand& command) {
...
bool CivilizationDeclineService::erase(std::uint64_t subjectId) {
...
void CivilizationDeclineService::advance(std::uint64_t tick, double delta) {
...
const CivilizationDeclineState* CivilizationDeclineService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CivilizationDeclineState* CivilizationDeclineService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CivilizationDeclineState> CivilizationDeclineService::ordered() const {
    auto result = states_;
...
std::vector<CivilizationDeclineEvent> CivilizationDeclineService::drainEvents() {
...
void CivilizationDeclineService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CivilizationDeclineService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
