...
#include "civilization/FrontierExpansion.hpp"
...
bool FrontierExpansionService::apply(const FrontierExpansionCommand& command) {
...
bool FrontierExpansionService::erase(std::uint64_t subjectId) {
...
void FrontierExpansionService::advance(std::uint64_t tick, double delta) {
...
const FrontierExpansionState* FrontierExpansionService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
FrontierExpansionState* FrontierExpansionService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<FrontierExpansionState> FrontierExpansionService::ordered() const {
    auto result = states_;
...
std::vector<FrontierExpansionEvent> FrontierExpansionService::drainEvents() {
...
void FrontierExpansionService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void FrontierExpansionService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
