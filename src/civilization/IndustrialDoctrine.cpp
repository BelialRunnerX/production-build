...
#include "civilization/IndustrialDoctrine.hpp"
...
bool IndustrialDoctrineService::apply(const IndustrialDoctrineCommand& command) {
...
bool IndustrialDoctrineService::erase(std::uint64_t subjectId) {
...
void IndustrialDoctrineService::advance(std::uint64_t tick, double delta) {
...
const IndustrialDoctrineState* IndustrialDoctrineService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
IndustrialDoctrineState* IndustrialDoctrineService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<IndustrialDoctrineState> IndustrialDoctrineService::ordered() const {
    auto result = states_;
...
std::vector<IndustrialDoctrineEvent> IndustrialDoctrineService::drainEvents() {
...
void IndustrialDoctrineService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void IndustrialDoctrineService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
