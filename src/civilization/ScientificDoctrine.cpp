...
#include "civilization/ScientificDoctrine.hpp"
...
bool ScientificDoctrineService::apply(const ScientificDoctrineCommand& command) {
...
bool ScientificDoctrineService::erase(std::uint64_t subjectId) {
...
void ScientificDoctrineService::advance(std::uint64_t tick, double delta) {
...
const ScientificDoctrineState* ScientificDoctrineService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ScientificDoctrineState* ScientificDoctrineService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ScientificDoctrineState> ScientificDoctrineService::ordered() const {
    auto result = states_;
...
std::vector<ScientificDoctrineEvent> ScientificDoctrineService::drainEvents() {
...
void ScientificDoctrineService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ScientificDoctrineService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
