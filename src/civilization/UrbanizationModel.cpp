...
#include "civilization/UrbanizationModel.hpp"
...
bool UrbanizationModelService::apply(const UrbanizationModelCommand& command) {
...
bool UrbanizationModelService::erase(std::uint64_t subjectId) {
...
void UrbanizationModelService::advance(std::uint64_t tick, double delta) {
...
const UrbanizationModelState* UrbanizationModelService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
UrbanizationModelState* UrbanizationModelService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<UrbanizationModelState> UrbanizationModelService::ordered() const {
    auto result = states_;
...
std::vector<UrbanizationModelEvent> UrbanizationModelService::drainEvents() {
...
void UrbanizationModelService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void UrbanizationModelService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
