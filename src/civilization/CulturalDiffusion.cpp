...
#include "civilization/CulturalDiffusion.hpp"
...
bool CulturalDiffusionService::apply(const CulturalDiffusionCommand& command) {
...
bool CulturalDiffusionService::erase(std::uint64_t subjectId) {
...
void CulturalDiffusionService::advance(std::uint64_t tick, double delta) {
...
const CulturalDiffusionState* CulturalDiffusionService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CulturalDiffusionState* CulturalDiffusionService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CulturalDiffusionState> CulturalDiffusionService::ordered() const {
    auto result = states_;
...
std::vector<CulturalDiffusionEvent> CulturalDiffusionService::drainEvents() {
...
void CulturalDiffusionService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CulturalDiffusionService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
