// Intended function: Track prosperity-driven golden ages, institutional momentum, and eventual stagnation pressure.
#include "civilization/GoldenAgeCycle.hpp"
...
bool GoldenAgeCycleService::apply(const GoldenAgeCycleCommand& command) {
...
bool GoldenAgeCycleService::erase(std::uint64_t subjectId) {
...
void GoldenAgeCycleService::advance(std::uint64_t tick, double delta) {
...
const GoldenAgeCycleState* GoldenAgeCycleService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
GoldenAgeCycleState* GoldenAgeCycleService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<GoldenAgeCycleState> GoldenAgeCycleService::ordered() const {
    auto result = states_;
...
std::vector<GoldenAgeCycleEvent> GoldenAgeCycleService::drainEvents() {
...
void GoldenAgeCycleService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void GoldenAgeCycleService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
