#include "strategy/StrategicWarGoals.hpp"
...
bool StrategicWarGoalsSystem::apply(const StrategicWarGoalsRequest& request) {
...
bool StrategicWarGoalsSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const StrategicWarGoalsState* StrategicWarGoalsSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> StrategicWarGoalsSystem::activeIds() const {
...
void StrategicWarGoalsSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
