#include "strategy/FederationSystem.hpp"
...
bool FederationSystemSystem::apply(const FederationSystemRequest& request) {
...
bool FederationSystemSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const FederationSystemState* FederationSystemSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> FederationSystemSystem::activeIds() const {
...
void FederationSystemSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
