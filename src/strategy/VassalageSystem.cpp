#include "strategy/VassalageSystem.hpp"
...
bool VassalageSystemSystem::apply(const VassalageSystemRequest& request) {
...
bool VassalageSystemSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const VassalageSystemState* VassalageSystemSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> VassalageSystemSystem::activeIds() const {
...
void VassalageSystemSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
