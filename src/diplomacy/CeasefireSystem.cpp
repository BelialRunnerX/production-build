...
#include "diplomacy/CeasefireSystem.hpp"
...
bool CeasefireSystemService::apply(const CeasefireSystemCommand& command) {
...
bool CeasefireSystemService::erase(std::uint64_t subjectId) {
...
void CeasefireSystemService::advance(std::uint64_t tick, double delta) {
...
const CeasefireSystemState* CeasefireSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CeasefireSystemState* CeasefireSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CeasefireSystemState> CeasefireSystemService::ordered() const {
    auto result = states_;
...
std::vector<CeasefireSystemEvent> CeasefireSystemService::drainEvents() {
...
void CeasefireSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CeasefireSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
