...
#include "diplomacy/AllianceCohesion.hpp"
...
bool AllianceCohesionService::apply(const AllianceCohesionCommand& command) {
...
bool AllianceCohesionService::erase(std::uint64_t subjectId) {
...
void AllianceCohesionService::advance(std::uint64_t tick, double delta) {
...
const AllianceCohesionState* AllianceCohesionService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
AllianceCohesionState* AllianceCohesionService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<AllianceCohesionState> AllianceCohesionService::ordered() const {
    auto result = states_;
...
std::vector<AllianceCohesionEvent> AllianceCohesionService::drainEvents() {
...
void AllianceCohesionService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void AllianceCohesionService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
