...
#include "diplomacy/ReputationPropagation.hpp"
...
bool ReputationPropagationService::apply(const ReputationPropagationCommand& command) {
...
bool ReputationPropagationService::erase(std::uint64_t subjectId) {
...
void ReputationPropagationService::advance(std::uint64_t tick, double delta) {
...
const ReputationPropagationState* ReputationPropagationService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ReputationPropagationState* ReputationPropagationService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ReputationPropagationState> ReputationPropagationService::ordered() const {
    auto result = states_;
...
std::vector<ReputationPropagationEvent> ReputationPropagationService::drainEvents() {
...
void ReputationPropagationService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ReputationPropagationService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
