...
#include "diplomacy/InfluenceNetwork.hpp"
...
bool InfluenceNetworkService::apply(const InfluenceNetworkCommand& command) {
...
bool InfluenceNetworkService::erase(std::uint64_t subjectId) {
...
void InfluenceNetworkService::advance(std::uint64_t tick, double delta) {
...
const InfluenceNetworkState* InfluenceNetworkService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
InfluenceNetworkState* InfluenceNetworkService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<InfluenceNetworkState> InfluenceNetworkService::ordered() const {
    auto result = states_;
...
std::vector<InfluenceNetworkEvent> InfluenceNetworkService::drainEvents() {
...
void InfluenceNetworkService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void InfluenceNetworkService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
