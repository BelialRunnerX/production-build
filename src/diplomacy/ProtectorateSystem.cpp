// Intended function: Track protectorate relationships, tribute, autonomy, and intervention authority.
#include "diplomacy/ProtectorateSystem.hpp"
...
bool ProtectorateSystemService::apply(const ProtectorateSystemCommand& command) {
...
bool ProtectorateSystemService::erase(std::uint64_t subjectId) {
...
void ProtectorateSystemService::advance(std::uint64_t tick, double delta) {
...
const ProtectorateSystemState* ProtectorateSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ProtectorateSystemState* ProtectorateSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ProtectorateSystemState> ProtectorateSystemService::ordered() const {
    auto result = states_;
...
std::vector<ProtectorateSystemEvent> ProtectorateSystemService::drainEvents() {
...
void ProtectorateSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ProtectorateSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
