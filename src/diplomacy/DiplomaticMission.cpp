...
#include "diplomacy/DiplomaticMission.hpp"
...
bool DiplomaticMissionService::apply(const DiplomaticMissionCommand& command) {
...
bool DiplomaticMissionService::erase(std::uint64_t subjectId) {
...
void DiplomaticMissionService::advance(std::uint64_t tick, double delta) {
...
const DiplomaticMissionState* DiplomaticMissionService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DiplomaticMissionState* DiplomaticMissionService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DiplomaticMissionState> DiplomaticMissionService::ordered() const {
    auto result = states_;
...
std::vector<DiplomaticMissionEvent> DiplomaticMissionService::drainEvents() {
...
void DiplomaticMissionService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DiplomaticMissionService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
