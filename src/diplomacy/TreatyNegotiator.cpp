...
#include "diplomacy/TreatyNegotiator.hpp"
...
bool TreatyNegotiatorService::apply(const TreatyNegotiatorCommand& command) {
...
bool TreatyNegotiatorService::erase(std::uint64_t subjectId) {
...
void TreatyNegotiatorService::advance(std::uint64_t tick, double delta) {
...
const TreatyNegotiatorState* TreatyNegotiatorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
TreatyNegotiatorState* TreatyNegotiatorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<TreatyNegotiatorState> TreatyNegotiatorService::ordered() const {
    auto result = states_;
...
std::vector<TreatyNegotiatorEvent> TreatyNegotiatorService::drainEvents() {
...
void TreatyNegotiatorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void TreatyNegotiatorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
