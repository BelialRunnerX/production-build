// Intended function: Represent defense guarantees and trigger obligations when protected parties are attacked.
#include "diplomacy/GuaranteeSystem.hpp"
...
bool GuaranteeSystemService::apply(const GuaranteeSystemCommand& command) {
...
bool GuaranteeSystemService::erase(std::uint64_t subjectId) {
...
void GuaranteeSystemService::advance(std::uint64_t tick, double delta) {
...
const GuaranteeSystemState* GuaranteeSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
GuaranteeSystemState* GuaranteeSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<GuaranteeSystemState> GuaranteeSystemService::ordered() const {
    auto result = states_;
...
std::vector<GuaranteeSystemEvent> GuaranteeSystemService::drainEvents() {
...
void GuaranteeSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void GuaranteeSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
