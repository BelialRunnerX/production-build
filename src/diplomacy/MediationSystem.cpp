...
#include "diplomacy/MediationSystem.hpp"
...
bool MediationSystemService::apply(const MediationSystemCommand& command) {
...
bool MediationSystemService::erase(std::uint64_t subjectId) {
...
void MediationSystemService::advance(std::uint64_t tick, double delta) {
...
const MediationSystemState* MediationSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
MediationSystemState* MediationSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<MediationSystemState> MediationSystemService::ordered() const {
    auto result = states_;
...
std::vector<MediationSystemEvent> MediationSystemService::drainEvents() {
...
void MediationSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void MediationSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
