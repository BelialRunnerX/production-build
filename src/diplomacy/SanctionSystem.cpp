...
#include "diplomacy/SanctionSystem.hpp"
...
bool SanctionSystemService::apply(const SanctionSystemCommand& command) {
...
bool SanctionSystemService::erase(std::uint64_t subjectId) {
...
void SanctionSystemService::advance(std::uint64_t tick, double delta) {
...
const SanctionSystemState* SanctionSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SanctionSystemState* SanctionSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SanctionSystemState> SanctionSystemService::ordered() const {
    auto result = states_;
...
std::vector<SanctionSystemEvent> SanctionSystemService::drainEvents() {
...
void SanctionSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SanctionSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
