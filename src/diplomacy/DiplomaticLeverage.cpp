...
#include "diplomacy/DiplomaticLeverage.hpp"
...
bool DiplomaticLeverageService::apply(const DiplomaticLeverageCommand& command) {
...
bool DiplomaticLeverageService::erase(std::uint64_t subjectId) {
...
void DiplomaticLeverageService::advance(std::uint64_t tick, double delta) {
...
const DiplomaticLeverageState* DiplomaticLeverageService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
DiplomaticLeverageState* DiplomaticLeverageService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<DiplomaticLeverageState> DiplomaticLeverageService::ordered() const {
    auto result = states_;
...
std::vector<DiplomaticLeverageEvent> DiplomaticLeverageService::drainEvents() {
...
void DiplomaticLeverageService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void DiplomaticLeverageService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
