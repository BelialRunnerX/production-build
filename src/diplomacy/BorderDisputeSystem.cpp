...
#include "diplomacy/BorderDisputeSystem.hpp"
...
bool BorderDisputeSystemService::apply(const BorderDisputeSystemCommand& command) {
...
bool BorderDisputeSystemService::erase(std::uint64_t subjectId) {
...
void BorderDisputeSystemService::advance(std::uint64_t tick, double delta) {
...
const BorderDisputeSystemState* BorderDisputeSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
BorderDisputeSystemState* BorderDisputeSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<BorderDisputeSystemState> BorderDisputeSystemService::ordered() const {
    auto result = states_;
...
std::vector<BorderDisputeSystemEvent> BorderDisputeSystemService::drainEvents() {
...
void BorderDisputeSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void BorderDisputeSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
