// Intended function: Track housing capacity, occupancy, crowding, quality, and construction demand by settlement.
#include "colony/HousingMarket.hpp"
...
bool HousingMarketService::apply(const HousingMarketCommand& command) {
...
bool HousingMarketService::erase(std::uint64_t subjectId) {
...
void HousingMarketService::advance(std::uint64_t tick, double delta) {
...
const HousingMarketState* HousingMarketService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
HousingMarketState* HousingMarketService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<HousingMarketState> HousingMarketService::ordered() const {
    auto result = states_;
...
std::vector<HousingMarketEvent> HousingMarketService::drainEvents() {
...
void HousingMarketService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void HousingMarketService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
