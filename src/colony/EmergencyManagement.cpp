...
#include "colony/EmergencyManagement.hpp"
...
bool EmergencyManagementService::apply(const EmergencyManagementCommand& command) {
...
bool EmergencyManagementService::erase(std::uint64_t subjectId) {
...
void EmergencyManagementService::advance(std::uint64_t tick, double delta) {
...
const EmergencyManagementState* EmergencyManagementService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
EmergencyManagementState* EmergencyManagementService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<EmergencyManagementState> EmergencyManagementService::ordered() const {
    auto result = states_;
...
std::vector<EmergencyManagementEvent> EmergencyManagementService::drainEvents() {
...
void EmergencyManagementService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void EmergencyManagementService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
