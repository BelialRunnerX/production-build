...
#include "colony/PublicHealthService.hpp"
...
bool PublicHealthServiceService::apply(const PublicHealthServiceCommand& command) {
...
bool PublicHealthServiceService::erase(std::uint64_t subjectId) {
...
void PublicHealthServiceService::advance(std::uint64_t tick, double delta) {
...
const PublicHealthServiceState* PublicHealthServiceService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PublicHealthServiceState* PublicHealthServiceService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PublicHealthServiceState> PublicHealthServiceService::ordered() const {
    auto result = states_;
...
std::vector<PublicHealthServiceEvent> PublicHealthServiceService::drainEvents() {
...
void PublicHealthServiceService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PublicHealthServiceService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
