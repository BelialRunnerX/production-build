...
#include "colony/ZoningSystem.hpp"
...
bool ZoningSystemService::apply(const ZoningSystemCommand& command) {
...
bool ZoningSystemService::erase(std::uint64_t subjectId) {
...
void ZoningSystemService::advance(std::uint64_t tick, double delta) {
...
const ZoningSystemState* ZoningSystemService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ZoningSystemState* ZoningSystemService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ZoningSystemState> ZoningSystemService::ordered() const {
    auto result = states_;
...
std::vector<ZoningSystemEvent> ZoningSystemService::drainEvents() {
...
void ZoningSystemService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ZoningSystemService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
