...
#include "colony/WaterAuthority.hpp"
...
bool WaterAuthorityService::apply(const WaterAuthorityCommand& command) {
...
bool WaterAuthorityService::erase(std::uint64_t subjectId) {
...
void WaterAuthorityService::advance(std::uint64_t tick, double delta) {
...
const WaterAuthorityState* WaterAuthorityService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
WaterAuthorityState* WaterAuthorityService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<WaterAuthorityState> WaterAuthorityService::ordered() const {
    auto result = states_;
...
std::vector<WaterAuthorityEvent> WaterAuthorityService::drainEvents() {
...
void WaterAuthorityService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void WaterAuthorityService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
