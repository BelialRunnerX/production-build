...
#include "colony/PublicTransit.hpp"
...
bool PublicTransitService::apply(const PublicTransitCommand& command) {
...
bool PublicTransitService::erase(std::uint64_t subjectId) {
...
void PublicTransitService::advance(std::uint64_t tick, double delta) {
...
const PublicTransitState* PublicTransitService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
PublicTransitState* PublicTransitService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<PublicTransitState> PublicTransitService::ordered() const {
    auto result = states_;
...
std::vector<PublicTransitEvent> PublicTransitService::drainEvents() {
...
void PublicTransitService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void PublicTransitService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
