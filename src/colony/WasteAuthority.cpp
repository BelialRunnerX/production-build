// Intended function: Track solid/liquid waste capacity, recycling, overflow, contamination, and disposal routes.
#include "colony/WasteAuthority.hpp"
...
bool WasteAuthorityService::apply(const WasteAuthorityCommand& command) {
...
bool WasteAuthorityService::erase(std::uint64_t subjectId) {
...
void WasteAuthorityService::advance(std::uint64_t tick, double delta) {
...
const WasteAuthorityState* WasteAuthorityService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
WasteAuthorityState* WasteAuthorityService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<WasteAuthorityState> WasteAuthorityService::ordered() const {
    auto result = states_;
...
std::vector<WasteAuthorityEvent> WasteAuthorityService::drainEvents() {
...
void WasteAuthorityService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void WasteAuthorityService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
