// Intended function: Allocate settlement revenue among services, infrastructure, defense, welfare, and reserves.
#include "colony/CivicBudget.hpp"
...
bool CivicBudgetService::apply(const CivicBudgetCommand& command) {
...
bool CivicBudgetService::erase(std::uint64_t subjectId) {
...
void CivicBudgetService::advance(std::uint64_t tick, double delta) {
...
const CivicBudgetState* CivicBudgetService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CivicBudgetState* CivicBudgetService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CivicBudgetState> CivicBudgetService::ordered() const {
    auto result = states_;
...
std::vector<CivicBudgetEvent> CivicBudgetService::drainEvents() {
...
void CivicBudgetService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CivicBudgetService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
