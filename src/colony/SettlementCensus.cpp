// Intended function: Maintain bounded aggregate demographic snapshots for planning and strategic simulation.
#include "colony/SettlementCensus.hpp"
...
bool SettlementCensusService::apply(const SettlementCensusCommand& command) {
...
bool SettlementCensusService::erase(std::uint64_t subjectId) {
...
void SettlementCensusService::advance(std::uint64_t tick, double delta) {
...
const SettlementCensusState* SettlementCensusService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SettlementCensusState* SettlementCensusService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SettlementCensusState> SettlementCensusService::ordered() const {
    auto result = states_;
...
std::vector<SettlementCensusEvent> SettlementCensusService::drainEvents() {
...
void SettlementCensusService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SettlementCensusService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
