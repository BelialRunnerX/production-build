...
#include "colony/CivicMaintenance.hpp"
...
bool CivicMaintenanceService::apply(const CivicMaintenanceCommand& command) {
...
bool CivicMaintenanceService::erase(std::uint64_t subjectId) {
...
void CivicMaintenanceService::advance(std::uint64_t tick, double delta) {
...
const CivicMaintenanceState* CivicMaintenanceService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CivicMaintenanceState* CivicMaintenanceService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CivicMaintenanceState> CivicMaintenanceService::ordered() const {
    auto result = states_;
...
std::vector<CivicMaintenanceEvent> CivicMaintenanceService::drainEvents() {
...
void CivicMaintenanceService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CivicMaintenanceService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
