#include "procedural/FactionGenerator.hpp"
...
bool FactionGeneratorSystem::apply(const FactionGeneratorRequest& request) {
...
bool FactionGeneratorSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const FactionGeneratorState* FactionGeneratorSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> FactionGeneratorSystem::activeIds() const {
...
void FactionGeneratorSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
