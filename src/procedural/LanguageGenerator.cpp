...
#include "procedural/LanguageGenerator.hpp"
...
bool LanguageGeneratorService::apply(const LanguageGeneratorCommand& command) {
...
bool LanguageGeneratorService::erase(std::uint64_t subjectId) {
...
void LanguageGeneratorService::advance(std::uint64_t tick, double delta) {
...
const LanguageGeneratorState* LanguageGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
LanguageGeneratorState* LanguageGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<LanguageGeneratorState> LanguageGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<LanguageGeneratorEvent> LanguageGeneratorService::drainEvents() {
...
void LanguageGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void LanguageGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
