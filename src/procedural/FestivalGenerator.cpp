// Intended function: Generate recurring festivals from seasons, victories, tragedies, harvests, and institutions.
#include "procedural/FestivalGenerator.hpp"
...
bool FestivalGeneratorService::apply(const FestivalGeneratorCommand& command) {
...
bool FestivalGeneratorService::erase(std::uint64_t subjectId) {
...
void FestivalGeneratorService::advance(std::uint64_t tick, double delta) {
...
const FestivalGeneratorState* FestivalGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
FestivalGeneratorState* FestivalGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<FestivalGeneratorState> FestivalGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<FestivalGeneratorEvent> FestivalGeneratorService::drainEvents() {
...
void FestivalGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void FestivalGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
