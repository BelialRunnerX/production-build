#include "procedural/StationGenerator.hpp"
...
bool StationGeneratorSystem::apply(const StationGeneratorRequest& request) {
...
bool StationGeneratorSystem::erase(std::uint64_t targetId) {
    return states_.erase(targetId) != 0;
}

const StationGeneratorState* StationGeneratorSystem::find(std::uint64_t targetId) const {
    const auto it = states_.find(targetId);
    return it == states_.end() ? nullptr : &it->second;
}

std::vector<std::uint64_t> StationGeneratorSystem::activeIds() const {
...
void StationGeneratorSystem::clear() {
    states_.clear();
    nextSequence_ = 1;
}
