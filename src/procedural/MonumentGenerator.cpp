// Intended function: Generate monuments tied to durable historical events, people, factions, and cultural motifs.
#include "procedural/MonumentGenerator.hpp"
...
bool MonumentGeneratorService::apply(const MonumentGeneratorCommand& command) {
...
bool MonumentGeneratorService::erase(std::uint64_t subjectId) {
...
void MonumentGeneratorService::advance(std::uint64_t tick, double delta) {
...
const MonumentGeneratorState* MonumentGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
MonumentGeneratorState* MonumentGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<MonumentGeneratorState> MonumentGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<MonumentGeneratorEvent> MonumentGeneratorService::drainEvents() {
...
void MonumentGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void MonumentGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
