// Intended function: Generate high-level settlement districts and connectivity from terrain and economy roles.
#include "procedural/SettlementLayoutGenerator.hpp"
...
bool SettlementLayoutGeneratorService::apply(const SettlementLayoutGeneratorCommand& command) {
...
bool SettlementLayoutGeneratorService::erase(std::uint64_t subjectId) {
...
void SettlementLayoutGeneratorService::advance(std::uint64_t tick, double delta) {
...
const SettlementLayoutGeneratorState* SettlementLayoutGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
SettlementLayoutGeneratorState* SettlementLayoutGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<SettlementLayoutGeneratorState> SettlementLayoutGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<SettlementLayoutGeneratorEvent> SettlementLayoutGeneratorService::drainEvents() {
...
void SettlementLayoutGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void SettlementLayoutGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
