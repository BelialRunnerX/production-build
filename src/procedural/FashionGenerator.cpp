...
#include "procedural/FashionGenerator.hpp"
...
bool FashionGeneratorService::apply(const FashionGeneratorCommand& command) {
...
bool FashionGeneratorService::erase(std::uint64_t subjectId) {
...
void FashionGeneratorService::advance(std::uint64_t tick, double delta) {
...
const FashionGeneratorState* FashionGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
FashionGeneratorState* FashionGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<FashionGeneratorState> FashionGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<FashionGeneratorEvent> FashionGeneratorService::drainEvents() {
...
void FashionGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void FashionGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
