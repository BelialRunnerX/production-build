...
#include "procedural/ArtifactLegendGenerator.hpp"
...
bool ArtifactLegendGeneratorService::apply(const ArtifactLegendGeneratorCommand& command) {
...
bool ArtifactLegendGeneratorService::erase(std::uint64_t subjectId) {
...
void ArtifactLegendGeneratorService::advance(std::uint64_t tick, double delta) {
...
const ArtifactLegendGeneratorState* ArtifactLegendGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ArtifactLegendGeneratorState* ArtifactLegendGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ArtifactLegendGeneratorState> ArtifactLegendGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<ArtifactLegendGeneratorEvent> ArtifactLegendGeneratorService::drainEvents() {
...
void ArtifactLegendGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ArtifactLegendGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
