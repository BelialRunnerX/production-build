...
#include "procedural/ArchitectureGenerator.hpp"
...
bool ArchitectureGeneratorService::apply(const ArchitectureGeneratorCommand& command) {
...
bool ArchitectureGeneratorService::erase(std::uint64_t subjectId) {
...
void ArchitectureGeneratorService::advance(std::uint64_t tick, double delta) {
...
const ArchitectureGeneratorState* ArchitectureGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ArchitectureGeneratorState* ArchitectureGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ArchitectureGeneratorState> ArchitectureGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<ArchitectureGeneratorEvent> ArchitectureGeneratorService::drainEvents() {
...
void ArchitectureGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ArchitectureGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
