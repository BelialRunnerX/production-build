...
#include "procedural/FactionDoctrineGenerator.hpp"
...
bool FactionDoctrineGeneratorService::apply(const FactionDoctrineGeneratorCommand& command) {
...
bool FactionDoctrineGeneratorService::erase(std::uint64_t subjectId) {
...
void FactionDoctrineGeneratorService::advance(std::uint64_t tick, double delta) {
...
const FactionDoctrineGeneratorState* FactionDoctrineGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
FactionDoctrineGeneratorState* FactionDoctrineGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<FactionDoctrineGeneratorState> FactionDoctrineGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<FactionDoctrineGeneratorEvent> FactionDoctrineGeneratorService::drainEvents() {
...
void FactionDoctrineGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void FactionDoctrineGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
