...
#include "procedural/ShipNameGenerator.hpp"
...
bool ShipNameGeneratorService::apply(const ShipNameGeneratorCommand& command) {
...
bool ShipNameGeneratorService::erase(std::uint64_t subjectId) {
...
void ShipNameGeneratorService::advance(std::uint64_t tick, double delta) {
...
const ShipNameGeneratorState* ShipNameGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
ShipNameGeneratorState* ShipNameGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<ShipNameGeneratorState> ShipNameGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<ShipNameGeneratorEvent> ShipNameGeneratorService::drainEvents() {
...
void ShipNameGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void ShipNameGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
