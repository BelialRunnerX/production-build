...
#include "procedural/RuinsGenerator.hpp"
...
bool RuinsGeneratorService::apply(const RuinsGeneratorCommand& command) {
...
bool RuinsGeneratorService::erase(std::uint64_t subjectId) {
...
void RuinsGeneratorService::advance(std::uint64_t tick, double delta) {
...
const RuinsGeneratorState* RuinsGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
RuinsGeneratorState* RuinsGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<RuinsGeneratorState> RuinsGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<RuinsGeneratorEvent> RuinsGeneratorService::drainEvents() {
...
void RuinsGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void RuinsGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
