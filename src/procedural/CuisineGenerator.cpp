...
#include "procedural/CuisineGenerator.hpp"
...
bool CuisineGeneratorService::apply(const CuisineGeneratorCommand& command) {
...
bool CuisineGeneratorService::erase(std::uint64_t subjectId) {
...
void CuisineGeneratorService::advance(std::uint64_t tick, double delta) {
...
const CuisineGeneratorState* CuisineGeneratorService::find(std::uint64_t subjectId) const {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
...
CuisineGeneratorState* CuisineGeneratorService::findMutable(std::uint64_t subjectId) {
    const auto it = std::find_if(states_.begin(), states_.end(), [subjectId](const auto& state) {
        return state.subjectId == subjectId;
    });
    return it == states_.end() ? nullptr : &*it;
...
std::vector<CuisineGeneratorState> CuisineGeneratorService::ordered() const {
    auto result = states_;
...
std::vector<CuisineGeneratorEvent> CuisineGeneratorService::drainEvents() {
...
void CuisineGeneratorService::clear() {
    states_.clear();
    events_.clear();
    ++revision_;
}

void CuisineGeneratorService::emit(std::uint64_t subjectId, std::uint64_t targetId, double magnitude,
